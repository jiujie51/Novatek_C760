...
#define NVT_FTG_MAX_LANDMARK    (68)
...
typedef enum {
	CODEC_VENC_FRAME_TYPE_P=0,
	CODEC_VENC_FRAME_TYPE_B,
	CODEC_VENC_FRAME_TYPE_I,
	CODEC_VENC_FRAME_TYPE_IDR,
	CODEC_VENC_FRAME_TYPE_KEY_P
} CodecVencFrameType;
...
#if 0
typedef struct {
	BOOL bCropEnable;           ///< Cropping enable or disable. NVT_TRUE means video encoder will crop area with target width and height from video source.
...
typedef struct {
    ...
    BOOL bEnable;               ///< enalbe/disable codec 3DNR function
} CodecVencNRSetting;
...
typedef struct {
	BOOL bNREnable;                ///< Audio noise reduction disable/enable
...
#define NVT_MD_USR_MAX_EVT_SIZE 10800
...
typedef struct {
	UINT32	uiTbg;
    USR_MD1_LV_INFO sLvInfo[MD_USR_MAX_LV_NUM];
	UINT32	reserved[6];
}USR_MD1_INFO;

typedef struct {
	UINT32  uiTimePeriod;
    UINT32	uiLvGmMd2Th[MD_USR_MAX_LV_NUM];
	UINT32	reserved[6];
}USR_MD2_INFO;

typedef struct {
	UINT32  uiUpdateProb;
    UINT32	uiUpdateNeighborEn;
	UINT32  uiDeghostEn;
	UINT32  uiThFeature;
	UINT32  uiSensi;
	UINT32	reserved[6];
...
typedef struct {
	...
	UINT32	uiTamperTh;
	UINT32  uiTamperFrmNum;
	UINT32  uiMdRst;
	UINT32  uiRefCellEn;
	UINT8   uCellMap[NVT_MD_USR_MAX_EVT_SIZE];
	UINT32  reserved[6];
} USR_GLOBAL_INFO;

typedef struct {
	USR_GLOBAL_INFO  sGlobalInfo;
	USR_MD1_INFO     sMd1Info;
	USR_MD2_INFO     sMd2Info;
    USR_MD3_INFO     sMd3Info;
	UINT32  		 reserved[6];
} USR_PARAM_INFO;
...
typedef struct{
    ...
    UINT32 uiJPEGAddr;    ///< face jepg address
    INT32 uiJPEGSize;    ///< face jpeg size
    UINT32 uiId;         ///< face id
...
typedef struct{
    ...
    UINT32      uiFullJPEGAddr; ///< Full picture address (jpeg)
    UINT32      uiFullJPEGSize; ///< Full picture size    (jpeg)
    UINT32      uiFullYUVAddr; ///< Full picture address (YUV)
    UINT32      uiFullYUVSize; ///< Full picture size    (YUV)
    CodecFTGFaceJPEGData sFaceData[MAX_FTGNUM]; ///<face picture data sturcture
...
/**
    ...
    @return NvtErrorCode (Refer to nvt_type.h)
*/
extern NVTRET NvtCodec_GetVencStream_Without_RequestIFrame(UINT32 uiVencChannelID, CodecVencStream *psVencStream, UINT32 uiTimeOutMs, BOOL bCopyBuf);
...
/**
    Get Audio encoded data with Sample length
    ...
    @param[in] uiTimeOutMs      Function time out, 0 means block mode(Unit is millisecond)
    @param[in] bCopyBuf         Payload data handle method, NVT_TRUE means copy data into additional memory buffer *
    @param[in] uiSampleByte     set Data length want to get, it should be refrence of uiAinG711Sample, uiSampleByte and  valueuiAinG711Sample should be the same

    @return NvtErrorCode (Refer to nvt_type.h)
*/
extern NVTRET NvtCodec_GetAinStreamBySample(UINT32 uiAinChannelID, CodecAencStream *psAencStream, UINT32 uiTimeOutMs, BOOL bCopyBuf,UINT32 uiSampleByte);
...
/*
    Get FTG Snapshot information

    @param[in] psFTGSnapData, the pointer to the data structure of CodecFTGSapshotData

    @return NvtErrorCode (Refer to nvt_type.h)
*/
extern ER NvtCOdec_GetFTGSnapshot(CodecFTGSapshotData *psFTGSnapData);

/**
    Get one snapshot that is YUV format

    @note
    -# This function must be used after video encoder start

    @param[in] psVencSnapshotConfig Pointer to the data structure of YUV snapshot configuration
    @param[out] psVencSnapshot      Pointer to the data structure of YUV snapshot

    @return NvtErrorCode (Refer to nvt_type.h)
*/
extern NVTRET NvtCodec_RequestYUV(CodecVencSnapshotConfig *psVencSnapshotConfig, CodecVencSnapshot *psVencSnapshot);

/**
    Release the YUV snapshot resource

    @param[in] void

    @return NvtErrorCode (Refer to nvt_type.h)
*/
extern NVTRET NvtCodec_ReleaseYUV(void);
