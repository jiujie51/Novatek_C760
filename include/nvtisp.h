...
typedef struct{
	...
	ISP_SHDR  eHDREnable;         ///< set HDR enable(1) or disable(0)
	UINT32 uiFrameNum;        ///< SHDR frame number, Range: 1 ~ 4
...
/*  Crop */
typedef struct{
	UINT32 	uiSensor;                  ///< Sensor ID
	UINT32 	uiIME;                     ///< IME Path ID
	BOOL	bEnable;                   ///< setting enable or disable
	URECT	sCropInfo;                 ///< Crop Info(x,y,w,h)
	UINT32 	scale_size_w;              ///< scale size width
	UINT32 	scale_size_h;              ///< scale size height
}ISPImageCropSetting;
