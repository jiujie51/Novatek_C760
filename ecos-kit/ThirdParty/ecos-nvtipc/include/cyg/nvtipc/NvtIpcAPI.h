...
typedef unsigned int        NVTIPC_BOOL;    ///< Unsigned 32 bits data type
...
/**
   ...
   The core ID.
*/
typedef enum _NVTIPC_CORE_ID {
	NVTIPC_CORE1 = 1,                 ///< MIPS core1
	NVTIPC_CORE2 = 2,                 ///< MIPS core2
	NVTIPC_CORE3 = 3,                 ///< DSP 1
	NVTIPC_MAX = NVTIPC_CORE3,
	ENUM_DUMMY4WORD(NVTIPC_CORE_ID)
} NVTIPC_CORE_ID;
...
/**
    Check if the address is cache or non-cached address.

    @param addr: cache or non-cached address.

    @return TRUE or FALSE
*/
extern NVTIPC_I32 NvtIPC_IsCacheAddr(NVTIPC_U32 addr);

/**
    Check if core is ready for send or receive IPC.

    @param core_id: the core id.

    @return TRUE or FALSE
*/
extern NVTIPC_BOOL NvtIPC_ChkIfCoreReady(NVTIPC_CORE_ID core_id);
