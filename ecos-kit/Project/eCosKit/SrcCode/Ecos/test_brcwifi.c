...
extern void wifi_escanresult_cb_fn(u32 status, void *data, int len);
...
void wifi_es_sender_callback_cb_fn(unsigned char *ip)
{
    printf("ES get sender ip %s\n", ip);
}
...
MAINFUNC_ENTRY(wifi,argc,argv)
{
    ...
    dhd_register_wifi_es_sender_callback(wifi_es_sender_callback_cb_fn);
    dhd_register_wifi_escanresult_callback(wifi_escanresult_cb_fn);
    ...
    if (argc == 2)
    {
        ...
        else if (strcmp(argv[1], "site_survey") == 0)
        {
            ...
            for (i = 0;i<num;i++){    
                char ssidbuf[129];
                wl_format_ssid(ssidbuf, list[i].ssid, strlen(list[i].ssid));
        ...
        else if (strcmp(argv[1], "associated") == 0)
        {
          ...
          return 0;
            ...
            wl_format_ssid(ssidbuf, status->ssid, strlen(status->ssid)); 
    ...
    else if (argc == 3)
    {
        ...
        else if (strcmp(argv[1], "ap") == 0)
        {
            ...
            if(1){
                ...
                v[c++] = "4";
                v[c] = 0;
                printf("wl ratesel_gul 4\n");
                wlu_main(c, v);         

                c = 0;
                v[c++] = "wl";
                v[c++] = "ampdu_dyn_txaggr";
                v[c++] = "0";
                v[c] = 0;
                printf("wl ampdu_dyn_txaggr 0\n");
                wlu_main(c, v);
            ...
            v[c++] = "1";
            v[c] = 0;
            printf("wl wme-ac ap be aifsn 1\n");
            wlu_main(3, v);
            ...
            v[c++] = "2";
            v[c] = 0;
            printf("wl wme_ac ap be ecwmin 2\n");
            wlu_main(3, v);
            ...
            v[c++] = "3";
            v[c] = 0;
            printf("wl wme_ac ap be ecwmax 3\n");
            wlu_main(3, v);
...
*/
        ...
        else if (strcmp(argv[1], "dhcpc") == 0)
        {
            ...
            if (argc == 5) {
                ...
                if (inet_aton(argv[2], &(param.own_ip)) == 0) {
                    printf("bad value: %s", argv[2]);
                }
                if (inet_aton(argv[3], &(param.server_ip)) == 0) {
