	...
	do {
		...
		if (info->flash_info->plane_flags && (uiRowAddr & (0x1 << 6))) {
			...
			*(u32*) 0xC00200B0 &= ~0x80000;
			...
			if (uiColAddr == info->flash_info->page_size) {
				nand_host_setup_transfer(info, &transParam, 1, info->flash_info->oob_size, _SINGLE_PAGE);

				flush_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + info->flash_info->oob_size));
			} else {
				nand_host_setup_transfer(info, &transParam, 1, info->flash_info->page_size + info->flash_info->oob_size, _SINGLE_PAGE);

				flush_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + info->flash_info->page_size + info->flash_info->oob_size));
			}
			...
			if (uiColAddr == info->flash_info->page_size)
				invalidate_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + info->flash_info->oob_size));
			else
