...
static ER pinmux_config_misc(uint32_t config);
...
static ER pinmux_config_sensor(uint32_t config)
{
	...
	if (config == PIN_SENSOR_CFG_NONE) {
		...
		tmp = config & (PIN_SENSOR_CFG_12BITS | PIN_SENSOR_CFG_MIPI
				| PIN_SENSOR_CFG_LVDS | PIN_SENSOR_CFG_CCIR8BITS | PIN_SENSOR_CFG_CCIR16BITS | PIN_SENSOR_CFG_12BITS_2ND);
		switch (tmp) {
#if 0
...
static ER pinmux_config_sensor2(uint32_t config)
{
	...
	if (config == PIN_SENSOR2_CFG_NONE) {
		...
		tmp = config & (PIN_SENSOR2_CFG_CCIR8BITS | PIN_SENSOR2_CFG_10BITS | PIN_SENSOR2_CFG_CCIR16BITS | PIN_SENSOR2_CFG_MIPI | PIN_SENSOR2_CFG_LVDS|PIN_SENSOR2_CFG_12BITS);
		switch (tmp) {
			...
			if (config & PIN_UART_CFG_CH2_2ND) {
				...
				if (config & PIN_UART_CFG_CH2_CTSRTS) {
					if (top_reg7.Bit.PI_CNT2 == PICNT_ENUM_PICNT) {
						printf("UART2 conflict with PICNT2_1: 0x%x\r\n", config);
						return E_OBJ;
					}
		...
		if (config & PIN_UART_CFG_CH3) {
			if (config & PIN_UART_CFG_CH3_2ND) {
				...
				if (top_reg5.Bit.SPI2 == SPI2_2ND_PINMUX) {
					printf("UART3_2 conflict with to SPI2_2: 0x%x\r\n", config);
					return E_OBJ;
				}
				...
				if (top_reg5.Bit.SPI3 == SPI_2ND_PINMUX) {
					printf("UART3 conflict with to SPI_2: 0x%x\r\n", config);
					return E_OBJ;
				}

				if (top_reg7.Bit.PWM12 == PWM_ENUM_PWM) {
					printf("UART3 conflict with to PWM12: 0x%x\r\n", config);
					return E_OBJ;
				}
				if (top_reg7.Bit.PWM13 == PWM_ENUM_PWM) {
					printf("UART3 conflict with to PWM13: 0x%x\r\n", config);
					return E_OBJ;
				}
				if (top_reg5.Bit.SIFCH3 == SIFCH_ENUM_SIF) {
					printf("UART3 conflict with to SIF3: 0x%x\r\n", config);
					return E_OBJ;
				}
				if (top_reg6.Bit.SP2_CLK == SP2_CLK_SEL_ENUM_SP2_2_CLK) {
				...
				if (config & PIN_UART_CFG_CH3_CTSRTS) {
					if (top_reg7.Bit.PWM14 == PWM_ENUM_PWM) {
						printf("UART3 RTS conflict with to PWM14: 0x%x\r\n", config);
						return E_OBJ;
					}
	...
	if (config == PIN_SPI_CFG_NONE) {
		...
		if (config & (PIN_SPI_CFG_CH2|PIN_SPI_CFG_CH2_2ND_PINMUX|PIN_SPI_CFG_CH2_2BITS)) {
			if (config & PIN_SPI_CFG_CH2_2ND_PINMUX) {
				if ((top_reg7.Bit.PWM8 == PWM_ENUM_PWM) || (top_reg3.Bit.ME_SHUT0_SEL == MES_SEL_ENUM_2ND_PINMUX)) {
					...
					return E_OBJ;
				} else if ((top_reg7.Bit.PWM10 == PWM_ENUM_PWM) || (top_reg3.Bit.ME2_SHUT0_SEL == MES_SEL_ENUM_2ND_PINMUX)) {
					printf("SPI2_2 conflict with PWM10 or ME2_2_SHUT0\r\n");
					return E_OBJ;
				} else if ((config & PIN_SPI_CFG_CH2_2BITS) && ((top_reg7.Bit.PWM11 == PWM_ENUM_PWM)||(top_reg3.Bit.ME2_SHUT1_SEL == MES_SEL_ENUM_2ND_PINMUX))) {
...
static ER pinmux_config_audio(uint32_t config)
{
	...
	if (config & (PIN_AUDIO_CFG_MCLK)) {
		...
		if (top_reg7.Bit.PI_CNT3 == PICNT_ENUM_PICNT) {
			printf("AUDIO conflict with PICNT3_1\r\n");
			return E_OBJ;
		}
		top_reg_pgpio0.Bit.PGPIO_19 = GPIO_ID_EMUM_FUNC;
...
static ER pinmux_config_misc(uint32_t config)
{
	...
	if (config & PIN_MISC_CFG_RTCLK) {
		...
		//check PICNT3_1
		if (top_reg7.Bit.PI_CNT3 == PICNT_ENUM_PICNT) {
			printf("RTC CLK conflict with PICNT3_1\r\n");
			return E_OBJ;
		}

		//check I2S_MCLK
		if (top_reg6.Bit.AUDIO_MCLK == AUDIO_MCLK_MCLK) {
			printf("RTC CLK conflict with AUDIO MCLK\r\n");
			return E_OBJ;
		}
	...
	if (config & PIN_MISC_CFG_SP2CLK_NONE) {
		...
		//check remote
		if (top_reg6.Bit.REMOTE == REMOTE_ENUM_REMOTE) {
			printf("SP_CLK2 conflict with to Remote: 0x%x\r\n", config);
			return E_OBJ;
		}
		//check PICNT1_1
		if (top_reg7.Bit.PI_CNT == PICNT_ENUM_PICNT) {
		...
		//check DMCLK
		if (top_reg6.Bit.DIGITAL_MIC == DMIC_ENUM_DMIC) {
			printf("SP_CLK2 conflict with to DM_CLK: 0x%x\r\n", config);
			return E_OBJ;
		}
		...
		//check PWM12
		if (top_reg7.Bit.PWM12 == PWM_ENUM_PWM) {
			printf("SP_CLK2_2 conflict with to PWM12: 0x%x\r\n", config);
			return E_OBJ;
		}
