...
int nvt_runfw_update_over_eth(void)
{
	...
	char cmd[128]={0};
	sprintf(cmd, "eth_init");
	run_command(cmd, 0);
	sprintf(cmd, "tftp 0x0180000 EthcamTxFW.bin");
	run_command(cmd, 0);
	sprintf(cmd, "nvt_update_all 0x180000 0");
	run_command(cmd, 0);
...
int nvt_boot_check_wdt_udata(void)
{
	...
	printf("WDT_DATA_OFS=0x%x\n",uiUserData);
	if(uiUserData == MAKEFOURCC('E','C','T','X')){
		return 1;
	}
#endif
	return 0;
...
static int nvt_boot_itron_bin_auto(void)
{
	...
	if (gHeadInfo.BinCtrl & 0x2) {
		...
		if (nvt_detect_fw_tbin()) {
			...
			#if defined(CONFIG_NVT_LINUX_SPINAND_BOOT)
			sprintf(cmd, "nand read 0x%x uitron 0x%x ", CONFIG_ITRON_SDRAM_BASE, test_preload_size);
			#elif defined(CONFIG_NVT_LINUX_SPINOR_BOOT)
			get_part("uitron", &part_off, &part_size);
			sprintf(cmd, "sf read 0x%x 0x%llx 0x%x ", CONFIG_ITRON_SDRAM_BASE, part_off, test_preload_size);
			#elif defined(CONFIG_NVT_LINUX_EMMC_BOOT)
			sprintf(cmd, "mmc read 0x%x 0x%llx 0x4", CONFIG_ITRON_SDRAM_BASE, part_off);
			#endif /* CONFIG_NVT_LINUX_SPINAND_BOOT */
			printf("=>run cmd : %s\n", cmd);
			run_command(cmd, 0);
			// here must flush to avoid cpu2' auto-flush overwriting to cpu1's uitron firmware area
			flush_dcache_range((u32)CONFIG_ITRON_SDRAM_BASE, (u32)(CONFIG_ITRON_SDRAM_BASE+test_preload_size));

			NVTPACK_BFC_HDR *pbfc = (NVTPACK_BFC_HDR *)CONFIG_ITRON_SDRAM_BASE;
			/* Boot compressed uitron or non-compressed uitron */
			if(pbfc->uiFourCC == MAKEFOURCC('B', 'C', 'L', '1')) {
				...
				if (CONFIG_ITRON_SDRAM_BASE < CONFIG_UBOOT_SDRAM_BASE && CONFIG_UBOOT_SDRAM_BASE < (CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE)) {
					tmp_addr = CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE - firmware_size - CONFIG_UBOOT_SDRAM_SIZE ;
				} else
					tmp_addr = CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE - firmware_size;
				if ((firmware_size + cpu_to_be32(pbfc->uiSizeUnComp)) > CONFIG_ITRON_SDRAM_SIZE) {
					printf("%s Attention!!! iTron starting address at 0x%08x with size 0x%08x (uncompressed) is larger than temp buffer address!! 0x%08x %s\r\n",
						ANSI_COLOR_RED, CONFIG_ITRON_SDRAM_BASE, cpu_to_be32(pbfc->uiSizeUnComp), tmp_addr, ANSI_COLOR_RESET);
					printf("%s It can't decompress %s\r\n", ANSI_COLOR_RED, ANSI_COLOR_RESET);
					while(1);
				}

				#if defined(CONFIG_NVT_LINUX_SPINAND_BOOT)
				sprintf(cmd, "nand read 0x%x uitron 0x%x ", tmp_addr, firmware_size);
				#elif defined(CONFIG_NVT_LINUX_SPINOR_BOOT)
				...
				if((cpu_to_be32(pbfc->uiAlgorithm) & 0xFF ) == 11)
				{
					...
					ret = lzmaBuffToBuffDecompress((unsigned char *)CONFIG_ITRON_SDRAM_BASE, &Uncompsz, (unsigned char *)(tmp_addr + sizeof(NVTPACK_BFC_HDR)), Compsz);
					if(ret != 0){
						printf("Decompressed lzma fail\n");
						#if (defined(_NVT_ETHREARCAM_TX_))
						return ret;
				...
				{
					nvt_writel((ulong)tmp_addr, (ulong)nvt_binfo_comm_itron_comp_addr);
					nvt_writel((ulong)firmware_size, (ulong)nvt_binfo_comm_itron_comp_len);
					flush_dcache_range((u32)ALIGN_FLOOR(tmp_addr, 64),
							(u32)ALIGN_CEIL((tmp_addr + firmware_size), 64));
					mdelay(500);
				}
			} else {
				NVTPACK_BININFO_HDR *pbininfo =  (NVTPACK_BININFO_HDR *)(CONFIG_ITRON_SDRAM_BASE + BIN_INFO_OFFSET_UITRON);
...
static int nvt_boot_fw_init_detect(void)
{
	...
	if(nvt_boot_check_wdt_udata()){
		printf("nvt_boot_check_wdt_udata Error!\n");
	...
	ret=nvt_boot_itron_bin_auto();
	#if (defined(_NVT_ETHREARCAM_TX_))
	if(ret != 0){
		printf("nvt_boot_itron_bin_auto Error!\n");
		nvt_runfw_update_over_eth();
	}
	#endif
	if (type == CPU2_OS_TYPE_NONE)
		...
		#if (defined(_NVT_ETHREARCAM_TX_))
		if(ret != 0){
			printf("nvt_boot_ecos_bin_auto Error!\n");
			nvt_runfw_update_over_eth();
			return ret;
		}
		#else
...
static int nvt_boot_itron_bin(void)
{
	...
	if (gHeadInfo.BinCtrl & 0x2) {
...
#if defined(CONFIG_NVT_LINUX_SPINAND_BOOT)
		char cmd[128] = {0};
		sprintf(cmd, "nand read 0x%x uitron 0x%x ", CONFIG_ITRON_SDRAM_BASE, 0x200);
		printf("=>run cmd : %s\n", cmd);
		run_command(cmd, 0);
		NVTPACK_BFC_HDR *pbfc = (NVTPACK_BFC_HDR *)CONFIG_ITRON_SDRAM_BASE;
		...
		if(pbfc->uiFourCC == MAKEFOURCC('B', 'C', 'L', '1')) {
			...
			/*
			 * Trigger compressed uitron boot.
			 * Comp. uitron => CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE - firmware_size;,
			 * UnComp. uitron => CONFIG_ITRON_SDRAM_BASE.
			 */
			size_t firmware_size = cpu_to_be32(pbfc->uiSizeComp) + sizeof(NVTPACK_BFC_HDR);
			if (CONFIG_ITRON_SDRAM_BASE < CONFIG_UBOOT_SDRAM_BASE && CONFIG_UBOOT_SDRAM_BASE < (CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE))
					tmp_addr = CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE - firmware_size - CONFIG_UBOOT_SDRAM_BASE;
			else
					tmp_addr = CONFIG_ITRON_SDRAM_BASE + CONFIG_ITRON_SDRAM_SIZE - firmware_size;
			if ((firmware_size + cpu_to_be32(pbfc->uiSizeUnComp)) > CONFIG_ITRON_SDRAM_SIZE) {
				printf("%s Attention!!! UITRON starting address at 0x%08x with size 0x%08x (uncompressed) is larger than temp buffer address!! 0x%08x %s\r\n",
					ANSI_COLOR_RED, CONFIG_ITRON_SDRAM_BASE, cpu_to_be32(pbfc->uiSizeUnComp), tmp_addr, ANSI_COLOR_RESET);
				printf("%s It can't decompress %s\r\n", ANSI_COLOR_RED, ANSI_COLOR_RESET);
				while(1);
			}
		...
		loff_t part_off=0, part_size=0;
		get_part("uitron", &part_off, &part_size);
		sprintf(cmd, "sf read 0x%x 0x%llx 0x%x ", CONFIG_ITRON_SDRAM_BASE, part_off, 0x200);
		printf("=>run cmd : %s\n", cmd);
		run_command(cmd, 0);
		NVTPACK_BFC_HDR *pbfc = (NVTPACK_BFC_HDR *)CONFIG_ITRON_SDRAM_BASE;
