...
static UINT16 MDAccLumAVG2[MD_LAWIN_TOTAL] = {0};
...
//#NT#2016/05/25#Harry_Tsai -end
    ...
    static UINT32 LPFCnt2 = 0;
    static BOOL prv_result2 = FALSE;
...
*/
    ...
    case 1:
        if(LPFCnt2 == 0)
        {
            for(BlockCnt = 0; BlockCnt < MD_LAWIN_TOTAL; BlockCnt ++)
            {
                MDAccLumAVG2[BlockCnt] = MDAccLum1[BlockCnt];
            }
            LPFCnt2 ++;
        }
        else
        {
            if(LPFCnt2 >= gMD_LPF_Cnt)
            {
                for(BlockCnt = 0; BlockCnt < MD_LAWIN_TOTAL; BlockCnt ++)
                {
                    MDAccLumAVG2[BlockCnt] /= LPFCnt2;
                }
                MdInfo[Id].CAWinX = WinXNum;
                MdInfo[Id].CAWinY = WinYNum;
                MdInfo[Id].AccTab[MDALG_CH_R] = MDAccLumAVG2;
                MdInfo[Id].AccTab[MDALG_CH_G] = MDAccLumAVG2;
                MdInfo[Id].AccTab[MDALG_CH_B] = MDAccLumAVG2;
                MdInfo[Id].BlockDiffThres = MD_BlkDiff_Thr[Id];
                MdInfo[Id].BlockCntThres = MD_BlkCnt_Thr[Id];
                MdInfo[Id].TotalDiffThres = MD_DiffSum_Thr[Id];
                MdInfo[Id].BlockDiffThres_Ref = MD_BlkDiff_Thr_Ref[Id];
