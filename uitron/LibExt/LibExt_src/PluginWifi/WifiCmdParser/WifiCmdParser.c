...
INT32 WifiCmd_GetData(char *path, char *argument, UINT32 bufAddr, UINT32 *bufSize, char *mimeType, UINT32 segmentCount)
{
	...
	UINT32            UserCB = 0;
	//char              *parStr;//post event,data in stack would release
	//DBG_IND("GetData path = %s, argument -> %s, mimeType= %s, bufsize= %d, segmentCount= %d\r\n", path, argument, mimeType, *bufSize, segmentCount);
...
INT32 WifiCmd_PutData(char *path, char *argument, UINT32 bufAddr, UINT32 bufSize, UINT32 segmentCount, UINT32 putStatus)  ///< Callback function for put custom data.
{
	...
	//DBG_IND("PutData path =%s, argument = %s, bufAddr = 0x%x, bufSize =0x%x , segmentCount  =%d , putStatus = %d\r\n", path, argument, bufAddr, bufSize, segmentCount, putStatus);
	...
	if (strncmp(argument, CMD_STR, strlen(CMD_STR)) == 0) {
		...
		if (UserCB) {
			...
			if (putStatus != WIFI_CMD_PUT_STATUS_CONTINUE) {
				WifiCmd_Unlock();
			}
			return hfs_result;
		} else {
...
void WifiCmdParser_SetDebug(BOOL DbgMsgOn)
{
	wificmdparser_debug = (DbgMsgOn > 0);
}
static BOOL Cmd_WifiCmdParser_SetDebug(CHAR *strCmd)
{
	UINT32 dbg_msg;

	sscanf_s(strCmd, "%d", &dbg_msg);

	wificmdparser_debug = (dbg_msg > 0);

	return TRUE;
}

SXCMD_BEGIN(wificmdparser, "wificmd parser")
SXCMD_ITEM("dbg %d",	Cmd_WifiCmdParser_SetDebug, 	"set debug msg (Param: 1->on, 0->off)")
SXCMD_END()

void WifiCmdParser_InstallCmd(void)
{
	static BOOL b_init = FALSE;
	if (!b_init) {
		SxCmd_AddTable(wificmdparser);
		b_init = TRUE;
	}
}
