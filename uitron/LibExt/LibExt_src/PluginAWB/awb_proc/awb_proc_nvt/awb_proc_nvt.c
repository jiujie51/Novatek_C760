...
static UINT16 awb_init_cnt[AWB_PROC_CNT] = {0};
static AWBALG_GAIN awb_pre_gain[AWB_PROC_CNT];
...
void AWB_Init(UINT32 id, UINT32 CurrentStatus)
{
	...
	if(awb_init_cnt[id] == 0) {
		awb_info[id].AWBGain = awb_info[id].DefGain;
	} else {
		awb_info[id].AWBGain = awb_pre_gain[id];
	}
...
void AWB_Process(UINT32 id, UINT32 CurrentStatus)
{
    ...
    if (ui_r_gain[id] != 0 && ui_g_gain[id] != 0 && ui_b_gain[id] != 0) {
		...
		if ((awb_info[id].AWBGain.r_gain < 256) || (awb_info[id].AWBGain.b_gain < 256)) {
			if (awb_info[id].AWBGain.r_gain < awb_info[id].AWBGain.b_gain) {
				if (awb_info[id].AWBGain.r_gain != 0) {
					awb_info[id].AWBGain.g_gain = (awb_info[id].AWBGain.g_gain << 8) / awb_info[id].AWBGain.r_gain;
					awb_info[id].AWBGain.b_gain = (awb_info[id].AWBGain.b_gain << 8) / awb_info[id].AWBGain.r_gain;
					awb_info[id].AWBGain.r_gain = 256;
				}
			} else {
				if (awb_info[id].AWBGain.b_gain != 0) {
					awb_info[id].AWBGain.g_gain = (awb_info[id].AWBGain.g_gain << 8) / awb_info[id].AWBGain.b_gain;
					awb_info[id].AWBGain.r_gain = (awb_info[id].AWBGain.r_gain << 8) / awb_info[id].AWBGain.b_gain;
					awb_info[id].AWBGain.b_gain = 256;
				}
			}
		}
	...
	awb_pre_gain[id] = awb_info[id].AWBGain;
