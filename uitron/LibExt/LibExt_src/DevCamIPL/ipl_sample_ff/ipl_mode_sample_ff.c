...
#if 0
			if (id == IPL_ID_1)
				sen_ad_param.b_manual_info = 1;
			else
				sen_ad_param.b_manual_info = 0;
#else
			//sen_ad_param.b_manual_info = IPL_AlgGetUIInfo(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID), IPL_SEL_AD_AUTO);
			sen_ad_param.b_manual_info = IPL_AlgGetUIInfo(id, IPL_SEL_AD_AUTO);
#endif
			sen_ad_param.fmt = IPL_AlgGetUIInfo(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID), IPL_SEL_AD_FMT);
			img_fps = IPL_AlgGetUIInfo(IPL_CtrlGetInfor(id, IPLCTRL_MF_BASE_ID), IPL_SEL_VIDEO_FPS);

			if (!sen_ad_param.b_manual_info) {
				sensor_get_info(id, SEN_GET_DET_PLUG, (void*)&plug_in_rlt);
				if (plug_in_rlt == TRUE){
					sensor_get_info(id, SEN_GET_DET_PLUG_INFO, (void*)&plug_in_info);
					img_size_h = plug_in_info.resolution.w;
					img_size_v = plug_in_info.resolution.h;
					img_fps = ((plug_in_info.fps/100)<<16) | 1;
				}
			}

			sen_ad_param.fps = img_fps;
			sen_ad_param.width = img_size_h;
			sen_ad_param.height = img_size_v;
			//DBG_ERR("ui size = %d %d, fps = 0x%x\r\n", img_size_h, img_size_v, img_fps);
