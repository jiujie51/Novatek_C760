...
// misc
//----------------------------------------------------------------------------------------------------------------------
void Lens_GetDriverName(INT8 *ic_name)
{
    memcpy(ic_name, "unknown", sizeof("unknown"));
    ic_name[sizeof("unknown")] = '\0';

    if (g_bLensSemaInit == TRUE) {
        if (g_LensCtrl.pLens->getDriverName) {
            g_LensCtrl.pLens->getDriverName(ic_name);
        } else {
...
{
    if (g_bLensSemaInit == TRUE) {
        if (g_LensCtrl.pLens->lensGetCapInfo) {
            return g_LensCtrl.pLens->lensGetCapInfo();
        } else {
            DBG_ERR("lens obj func is not registered!\r\n");
        }
    }
...
UINT32 Lens_GetInitState(void)
{
    if (g_bLensSemaInit == TRUE) {
        if (g_LensCtrl.pLens->getInitState) {
            return g_LensCtrl.pLens->getInitState();
        } else {
            DBG_ERR("lens obj func is not registered!\r\n");
        }
    }
...
INT32 Lens_ZoomMove(UINT32 move_mode)
{
    INT32 ret_status = ERR_OK;
    ...
    if (g_LensCtrl.pLens->zoom_press) {
        ...
        g_LensCtrl.pLens->zoom_press(move_mode);
...
INT32 Lens_FocusDoAction(INT32 position)
{
    INT32 move_step = 0;
...
INT32 Lens_FocusDoFreeAction(INT32 position)
{
    INT32 move_step = 0;
    ...
    if (g_LensCtrl.pLens->focus_go2Position) {
        ...
        if (g_LensCtrl.pLens->focus_go2Position) {
            ...
            move_step = g_LensCtrl.pLens->focus_go2Position(position, FALSE);
...
INT32 Lens_FocusMove(UINT32 move_mode)
{
    INT32 ret_status = ERR_OK;
    ...
    if (g_LensCtrl.pLens->focus_press) {
        ...
        g_LensCtrl.pLens->focus_press(move_mode);
...
INT32 Lens_ApertureSetLumFlux(UINT32 lum_flux)
{
    INT32 ret_status = ERR_OK;
    ...
    if (g_LensCtrl.pLens->aperture_setLumFlux) {
        Lens_ModuleSetState(LENS_STATE_APERTURE_MOVING, TRUE);
        DBG_MSG("LENS_STATE_APERTURE_MOVING\r\n");
        ...
        g_LensCtrl.pLens->aperture_setLumFlux(lum_flux);
