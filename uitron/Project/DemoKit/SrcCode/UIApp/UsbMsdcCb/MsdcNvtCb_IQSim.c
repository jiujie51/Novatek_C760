...
void IQSAF_set_force_stop(void);
...
void IQSISP_get_sie_img_str(void);
...
void IQSLENS_get_motor_sts(void);
void IQSLENS_get_zoom_focus_item(void);
void IQSLENS_get_zoom_focus_tbl(void);
void IQSLENS_get_init_sts(void);
void IQSLENS_get_drv_ic_name(void);
...
void IQSLENS_set_focus_free_pos(void);
...
static void (*MSDCIQSimGet[])(void) = {
	...
	IQSLENS_get_motor_sts,      // 164
	IQSLENS_get_zoom_focus_item,// 165
	IQSLENS_get_zoom_focus_tbl, // 166
	IQSLENS_get_init_sts,       // 167
	IQSLENS_get_drv_ic_name,    // 168
...
static void (*MSDCIQSimSet[])(void) = {
	...
	IQSLENS_set_focus_free_pos,  // 167
...
void IQSAF_set_force_stop(void)
{
    ...
    AFTuning_Set_Cmd(sensor_id, AF_TUNING_SET_FORCE_STOP, (UINT32)pData, uiLength);
}
...
void IQSISP_get_ime_info(void)
{
	...
	sPtr->ime_info.p3_enable = ImageApp_MovieMulti_IsStreamRunning(_CFG_STRM_ID_1);
...
void IQSISP_get_yuv_lock(void)
{
	...
	isf_out = ImageApp_MovieMulti_GetStreamingDataPort(_CFG_REC_ID_1);
#endif
...
void IQSISP_get_raw_info(void)
{
	...
	if (IPL_AlgGetUIInfo(IPL_ID_1, IPL_SEL_SHDR) == SEL_SHDR_ON) {
		...
		ipl_id = (sensor_id == 0) ? IPL_ID_1 : IPL_ID_2;
	}
...
void IQSISP_get_size_check(void)
{
	...
	// 6
	sISP_PATHCURVE *sPtr_pathcurve = (sISP_PATHCURVE *)pData;
	sPtr_pathcurve->Id = sensor_id;

	iqt_get_param(ISP_PATHCURVE, (UINT32)pData, sizeof(sISP_PATHCURVE));

	des_addr += sizeof(sISP_HDR_CONFIG);
	data_size += sizeof(sISP_PATHCURVE);

	memcpy(des_addr, pData, sizeof(sISP_PATHCURVE));
	...
	des_addr += sizeof(sISP_PATHCURVE);
...
void IQSISP_get_sie_img_str(void)
{
	...
	IPL_SIE_IO_SIZE_INFOR sie_io_size_info = {0};

	sie_img_str *img_str = (sie_img_str *)pData;

	sen_mode = IPL_CtrlGetInfor(sensor_id, IPLCTRL_SEN_MODE);
	sensor_status.sensor_mode = sen_mode;
	rt |= sensor_get_info(sensor_id, SEN_GET_STATUS, (void *)&sensor_status);
	if (rt != E_OK || sensor_status.sensor_info.mode == NULL) {
		return;
	}

	img_str->act_str_x = sensor_status.sensor_info.mode->trans_hd[0]->data_start;
	img_str->act_str_y = sensor_status.sensor_info.mode->trans_vd[0]->data_start;

	if (uiLength != sizeof(sie_img_str)) {
		DBG_ERR("size mismatch\r\n");
		return;
	}
	sie_io_size_info.id = sensor_id;

	if (IPL_GetCmd(IPL_GET_SIE_IO_SIZE_INFOR, (void *)&(sie_io_size_info)) != E_OK) {
		DBG_ERR("ipl get sie io size info fail\r\n");
		return;
	}
	img_str->crp_str_x = sie_io_size_info.crp_win.x;
	img_str->crp_str_y = sie_io_size_info.crp_win.y;
}
...
#if (IPCAM_FUNC==ENABLE)
	UINT32 *pData = (UINT32 *)MsdcNvt_GetDataBufferAddress();
	UINT32 i, mirror_flip = 0, img_dir = 0;
	IPCAM_STREAM_INFO *pStreamInfo = UIAppIPCam_get_streamInfo();
	IPL_SIE_CUR_FRAME_INFO sie_info = { 0 };
...
void IQSLENS_get_focus_range(void)
{
	...
	range_type = (FOCUS_RANGE)((lens_argu > FOCUS_RANGE_BOTH) ? FOCUS_RANGE_BOTH : lens_argu);

	pData[0] = Lens_FocusGetRange(range_type);
}
...
void IQSLENS_get_init_sts(void)
{
	...
	pData[0] = Lens_GetInitState();
}

void IQSLENS_get_drv_ic_name(void)
{
	INT8 *pData = (INT8 *)MsdcNvt_GetDataBufferAddress();

	Lens_GetDriverName(pData);
}
...
void IQSLENS_set_zoom_define_pos(void)
{
	...
	def_dir = ((pData[0] >> 16) >= 0) ? 1 : -1;

	Lens_ZoomSetPosition((pData[0] & 0xFFFF), def_dir);
}
...
void IQSLENS_set_focus_define_pos(void)
{
	...
	Lens_FocusSetPosition((pData[0] & 0xFFFF), def_dir);
}
...
void IQSLENS_set_focus_move(void)
{
	...
	move_mode = (pData[0] > FOCUS_TELE_SIDE) ? FOCUS_TELE_SIDE : pData[0];

	Lens_FocusSetFunc((LENS_FOCUS_WIDE + move_mode), 0);
}
...
void IQSLENS_set_focus_free_pos(void)
{
	...
	Lens_FocusSetFunc(LENS_FOCUS_GO2_FREE_POSITION, pData[0]);
}
