...
extern int UserSocket_Send(char *addr, int *size);
static BOOL Cmd_wifi_usnd(CHAR *strCmd)
{
    char snd_buf[512]={0};
    int size=0;
    sscanf_s(strCmd, "%s", snd_buf,sizeof(snd_buf));
    size = strlen(snd_buf);
    DBG_ERR("%s %d \r\n",snd_buf,size);
    UserSocket_Send(snd_buf,&size);
	return TRUE;
}
...
SXCMD_ITEM("usnd %", Cmd_wifi_usnd, "usocket snd")
SXCMD_END()
#endif
