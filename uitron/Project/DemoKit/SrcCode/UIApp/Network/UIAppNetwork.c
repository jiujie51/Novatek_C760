...
INT32 UINet_HFSUploadResultCb(int result, UINT32 bufAddr, UINT32 *bufSize, CHAR *mimeType, UINT32 segmentCount)
{
	int len;
#if _TODO
	XML_DefaultFormat(WIFIAPP_CMD_UPLOAD_FILE, result, bufAddr, (HFS_U32 *)bufSize, (char *)mimeType);
#else
	DBG_IND("segmentCount = %d\r\n", segmentCount);
	if (segmentCount == 0) {
		len = snprintf((char *)bufAddr, *bufSize, "Upload ok begin\r\n");
		*bufSize = len;
...
char gSSID[NVT_WSC_MAX_SSID_LEN] = DEFAULT_SSID;//"NVT_CARDV";
char gSSID_AP_default[NVT_WSC_MAX_SSID_LEN] = DEFAULT_SSID;//"NVT_CARDV";
char gPASSPHRASE[NVT_MAX_WEP_KEY_LEN] = DEFAULT_PSW;//"12345678";
char gPASSPHRASE_AP_default[NVT_MAX_WEP_KEY_LEN] = DEFAULT_PSW;//"12345678";
...
void UINet_WifiSettings(nvt_wifi_settings *pwifi, UINT32 mode, UINT32 security)
{
	...
	if (mode == NET_AP_MODE || mode == NET_WPS_AP_PBC_MODE) {
		...
		if (ptMenuStoreInfo->strSSID[0] == 0) {
			...
			snprintf(pwifi->ssid, NVT_WSC_MAX_SSID_LEN, "%s%02x%02x", gSSID, gMacAddr[4], gMacAddr[5]);
...
//#NT#2016/04/14#YongChang_Qui -end
	...
	for(i=0;i<ETH_REARCAM_CAPS_COUNT;i++){
		socketCliEthData2_SetResetI(i, 1);
		socketCliEthData2_SetAllowPush(i, 0);
	}
	#endif

	#if (ETH_REARCAM_CAPS_COUNT >= 2)
	UINT32 j;
	if(ImageApp_MovieMulti_IsStreamRunning(_CFG_ETHCAM_ID_1) || ImageApp_MovieMulti_IsStreamRunning(_CFG_ETHCAM_ID_2)){
...
//#NT#2016/08/25#YongChang_Qui -end
	...
	#if (ETH_REARCAM_CLONE_FOR_DISPLAY == ENABLE)
	for(i=0;i<ETH_REARCAM_CAPS_COUNT;i++){
		socketCliEthData2_SetAllowPush(i, 1);
	}
#endif
	#if (ETH_REARCAM_CAPS_COUNT >= 2)
	for(j=0;j<ETH_REARCAM_CAPS_COUNT;j++){
    ...
    if (result == E_OK && mode == NET_AP_MODE) {
		...
		#if defined(_NETWORK_ON_CPU1_)
		if (Sys_GetPrevWifiMode() == NET_STATION_MODE) {
			init_loopback_interface(0);
		}
		#endif
...
INT32 UINet_RtspInit(void)
{
	...
	runmaincmd(cmd);
#elif defined(_NETWORK_ON_CPU2_) && defined(_CPU2_ECOS_)
	char cmd[] = "nvtrtspd_ecos open";
