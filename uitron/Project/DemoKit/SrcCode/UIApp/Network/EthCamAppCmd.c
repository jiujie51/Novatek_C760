...
UINT8 EthcamTx_Timelapse ;
int EthCam_TxEnterTimelapse(char *path, char *argument, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType, HFS_U32 segmentCount)
{
	...
	UI_SetData(FL_MOVIE_TIMELAPSE_REC, par);
	DBGD(UI_GetData(FL_MOVIE_TIMELAPSE_REC));
	}
#endif
	XML_DefaultFormat(ETHCAM_CMD_TX_ENTER_TIMELAPSE, ETHCAM_RET_OK, bufAddr, bufSize, mimeType);
	return ETHCAM_CMD_GETDATA_RETURN_OK;
//	}
}

int EthCam_TxExitTimelapse(char *path, char *argument, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType, HFS_U32 segmentCount)
{
	debug_msg("path =%s, argument = %s, bufAddr = 0x%x, bufSize =0x%x , segmentCount  =%d , putStatus = %d\r\n", path, argument, bufAddr, bufSize, segmentCount);
#if(defined(_NVT_ETHREARCAM_RX_))
	CHKPNT;
	...
	//GxSystem_EnableHWResetByAlarmTime(0,0,0,3);
	//GxSystem_EnableSWReset(0);
	}
#endif
	XML_DefaultFormat(ETHCAM_CMD_TX_EXIT_TIMELAPSE, ETHCAM_RET_OK, bufAddr, bufSize, mimeType);
	return ETHCAM_CMD_GETDATA_RETURN_OK;
}
int EthCam_SetTxDATEIMPRINT(char *path, char *argument, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType, HFS_U32 segmentCount)
{
	debug_msg("path =%s, argument = %s, bufAddr = 0x%x, bufSize =0x%x , segmentCount  =%d , putStatus = %d\r\n", path, argument, bufAddr, bufSize, segmentCount);
#if defined(_NVT_ETHREARCAM_TX_)
	char  *pch = 0;
	...
	UI_SetData(FL_MOVIE_DATEIMPRINT, par);
	DBGD(UI_GetData(FL_MOVIE_DATEIMPRINT));
	}
#endif
	XML_DefaultFormat(ETHCAM_CMD_SET_TX_DATEIMPRINT, ETHCAM_RET_OK, bufAddr, bufSize, mimeType);

	return ETHCAM_CMD_GETDATA_RETURN_OK;
}

int EthCam_SetTxMontionDet(char *path, char *argument, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType, HFS_U32 segmentCount)
{
	debug_msg("path =%s, argument = %s, bufAddr = 0x%x, bufSize =0x%x , segmentCount  =%d , putStatus = %d\r\n", path, argument, bufAddr, bufSize, segmentCount);
#if defined(_NVT_ETHREARCAM_TX_)
	char  *pch = 0;
	...
	UI_SetData(FL_MOVIE_MOTION_DET, par);
	DBGD(UI_GetData(FL_MOVIE_MOTION_DET));	
	}
#endif
return ETHCAM_CMD_GETDATA_RETURN_OK;
}
...
void EthCam_ENTER_TIMELAPSE_CB(INT32 bEnd, void *output_data)
{
	ETHCAM_XML_DEFAULT_FORMAT *output=(ETHCAM_XML_DEFAULT_FORMAT*)output_data;
	DBG_DUMP("TxTimelapse cmd=%d, path_id=%d, port_type=%d, status=%d\r\n",output->cmd, output->path_id,output->port_type, output->status);
	CHKPNT;
	if(output->status == MOVIE_TIMELAPSEREC_1SEC)
		debug_msg("Ethcam Tx Enter Timelapse Mode ok!!\r\n");
	else
		debug_msg("Ethcam Tx Enter Timelapse Mode err!!\r\n");
}
...
ETHCAM_CMD_ITEM(ETHCAM_CMD_TX_ENTER_TIMELAPSE, 0, (UINT32)EthCam_TxEnterTimelapse ,0, FL_NULL)
ETHCAM_CMD_ITEM(ETHCAM_CMD_SET_TX_DATEIMPRINT, 0, (UINT32)EthCam_SetTxDATEIMPRINT ,0, FL_NULL)
ETHCAM_CMD_ITEM(ETHCAM_CMD_SET_TX_MOTIONDET, 0, (UINT32)EthCam_SetTxMontionDet ,0, FL_NULL)
ETHCAM_CMD_ITEM(ETHCAM_CMD_TX_EXIT_TIMELAPSE, 0, (UINT32)EthCam_TxExitTimelapse ,0, FL_NULL)
...
ETHCAM_XML_RESULT EthCamXMLResultTbl[]={
	...
	{ETHCAM_CMD_TX_ENTER_TIMELAPSE, 			ETHCAM_XML_RESULT_TYPE_DEFAULT_FORMAT,		NULL},
	{ETHCAM_CMD_SET_TX_DATEIMPRINT, 			ETHCAM_XML_RESULT_TYPE_DEFAULT_FORMAT,			NULL},
	{ETHCAM_CMD_SET_TX_MOTIONDET, 				ETHCAM_XML_RESULT_TYPE_DEFAULT_FORMAT,			NULL},
	{ETHCAM_CMD_TX_EXIT_TIMELAPSE, 				ETHCAM_XML_RESULT_TYPE_DEFAULT_FORMAT,			NULL},
	...
	{ETHCAM_CMD_TX_LCA_INFO, 					ETHCAM_XML_RESULT_TYPE_LIST, 					NULL},
