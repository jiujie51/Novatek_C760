...
#include "nvtmpp.h"
#if(WIFI_AP_FUNC==ENABLE || defined(_NVT_ETHREARCAM_TX_) || defined(_NVT_ETHREARCAM_RX_))
#include "nvt_auth.h"
...
void XML_DefaultFormat(UINT32 cmd, UINT32 ret, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType)
{
    ...
    xmlBufSize=strlen(DEF_XML_HEAD)+strlen(DEF_XML_RET);
    if(bufflen<xmlBufSize) {
        DBG_ERR("no buffer\r\n");
    } else {
...
int XML_GetDualCamShow(char *path, char *argument, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType, HFS_U32 segmentCount)
{
	UINT32 status = EthCamNet_GetEthLinkStatus(0);
	UINT32 val;
	...
	XML_ValueResult(WIFIAPP_CMD_GET_DAULCAM, val, bufAddr, bufSize, mimeType);
/*
	//0:single   1:dualcam
#if 1//(SENSOR_CAPS_COUNT >= 2)
	XML_ValueResult(WIFIAPP_CMD_GET_DAULCAM, 1, bufAddr, bufSize, mimeType);
#else
	XML_ValueResult(WIFIAPP_CMD_GET_DAULCAM, 0, bufAddr, bufSize, mimeType);
#endif
*/
	return CYG_HFS_CB_GETDATA_RETURN_OK;
}
...
int XML_GetMenuItem(char *path, char *argument, HFS_U32 bufAddr, HFS_U32 *bufSize, char *mimeType, HFS_U32 segmentCount)
{
	...
	while (appCmd[uiCmdIndex].cmd != 0) {
		if (appCmd[uiCmdIndex].UIflag != FL_NULL) {
			i = 0;
			if(appCmd[uiCmdIndex].UIflag == FL_MOVIE_TIMELAPSE_TIME)
				{
					debug_msg("zjf ----find appCmd and the flag is  FL_movie_timelapse_set \r\n");
				}
			while (wifiAppFlagMap[i].uiFlag != FL_NULL) {
				if (wifiAppFlagMap[i].uiFlag == appCmd[uiCmdIndex].UIflag) {
					if(appCmd[uiCmdIndex].UIflag == FL_MOVIE_TIMELAPSE_TIME)
						debug_msg("zjf ----find wifiappFlagMap flag, is FL_movie_timelapse_set \r\n");
					XML_snprintf(&buf, &xmlBufSize, "<Item>\n");
	...
	if (putStatus == HFS_PUT_STATUS_ERR) {
		...
		if (segmentCount == 0) {
			...
			if (uiUsage == FW_UPLOAD_USAGE_SAVE_AS_FILE) {
                ...
                if(fhdl){
                    ...
                    DBG_ERR("no file\r\n");
                    return CYG_HFS_UPLOAD_FAIL_FILENAME_EMPTY;
                }
            } else {
				uiCopiedBytes = 0;
		...
		if (putStatus == HFS_PUT_STATUS_FINISH) {
			if (uiUsage == FW_UPLOAD_USAGE_SAVE_AS_FILE) {
                ...
                UINT32 fw_align_size,unp_size = 0;
                BFC *pBfc = (BFC *)uiTempBuf;
                UINT32 uiSizeUnComp_le = UINT32_SWAP(pBfc->uiSizeUnComp);
                UINT32 uiSizeComp_le = UINT32_SWAP(pBfc->uiSizeComp);
                debug_msg("^GCopied bytes:%d %d\r\n", uiCopiedBytes,uiSizeComp_le);

                fw_align_size =ALIGN_CEIL_16(uiCopiedBytes);
                LZ_Uncompress((unsigned char*)uiTempBuf+sizeof(BFC),(unsigned char*)(uiTempBuf+fw_align_size),0x100);

                BININFO *bin_info= (BININFO *)(uiTempBuf+fw_align_size);

                if(bin_info->head.BinLength != uiSizeUnComp_le)
                {
                    debug_msg("FW len chk failed\r\n");
                    return UPDNAND_STS_FW_WRITE_CHK_ERR;
                }

                if(MAX_UPLOAD_FILE_SIZE-fw_align_size<uiSizeUnComp_le)
                {
                    debug_msg("buf %x < fw %x\r\n",MAX_UPLOAD_FILE_SIZE-fw_align_size,uiSizeUnComp_le);
                    return UPDNAND_STS_FW_WRITE_CHK_ERR;
                }

                if(fw_align_size-sizeof(BFC)<uiSizeComp_le)
                {
                    debug_msg("buf %x < unpress %x\r\n",fw_align_size-sizeof(BFC),uiSizeComp_le);
                    return UPDNAND_STS_FW_WRITE_CHK_ERR;
                }
                unp_size=LZ_Uncompress((unsigned char*)uiTempBuf+sizeof(BFC),(unsigned char*)(uiTempBuf+fw_align_size),uiSizeComp_le);

                if(MemCheck_CalcCheckSum16Bit(uiTempBuf+fw_align_size,unp_size)!=0)
                {
                    debug_msg("FW bin chk failed\r\n");
                    return UPDNAND_STS_FW_WRITE_CHK_ERR;
                }
#if 1
#if (FWS_FUNC == ENABLE)
