...
UINT32  gUIWifiAutoRecTimerID = NULL_TIMER;
...
//#NT#Parameter number must be larger then 0. Fixed mantis bug id:0107729
	...
	{
		Ux_CloseWindow(&UIMenuWndWiFiWaitCtrl,0);
	}
//#NT#2016/08/31#Isiah Chang -end
	...
	case WIFI_APP_MODE_PLAYBACK:
		debug_msg("currmode is playback\r\n");
...
INT32 WiFiCmd_OnExePipStyle(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	Ux_SendEvent(&CustomMovieObjCtrl,	NVTEVT_EXE_DUALCAM,	   1, Data);
#endif
	return NVTEVT_CONSUME;
}

//park factory test
static BOOL bWaitGsensorInt = FALSE;
BOOL IsWaitingGsensorInt(void)
{
    return bWaitGsensorInt;
}
void WaitForGsensorInt(BOOL bValue)
{
    bWaitGsensorInt = bValue;
}
//
extern  BOOL bDemoStart;
extern void GSensor_SetMovieParkSensitivity(UINT32 MovieParkSensitivity);
INT32 WiFiCmd_OnExeMoviePark(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{CHKPNT;
	DBG_IND("%s \r\n", __FUNCTION__);
	UINT32 Data = 0;
	if (paramNum) {
	...
	Ux_SendEvent(0, NVTEVT_EXE_MOVIE_PARK, 1, Data);
	
	if(bDemoStart)
	{CHKPNT;
		GSensor_SetMovieParkSensitivity(3);
	    GSensor_ParkingMode(TRUE);
		ClearIntSts();
	    WaitForGsensorInt(TRUE);
	}
	else
	{CHKPNT;
...
{	
    ...
    {
		debug_msg("Change to movie mode!\r\n");
		if (System_GetState(SYS_STATE_CURRMODE) != PRIMARY_MODE_MOVIE) {
			if(WifiCmd_GetCurCmd() == 0){
               Ux_PostEvent(NVTEVT_SYSTEM_MODE, 1, PRIMARY_MODE_MOVIE);
...
//#NT#2016/03/23#Isiah Chang -end
//#MT#2020/09/28#zjf -begin
INT32 WiFiCmd_OnExeTimeLapseTime(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	 DBG_IND("%s \r\n", __FUNCTION__);
	 UINT32 Data = 0;
	 if (paramNum) {
	 ...
	 SysSetFlag(FL_movie_timelapse_set, Data);
	 return NVTEVT_CONSUME;
}

//#MT#2020/09/28#zjf -end
...
EVENT_ENTRY UIWifiCmdObjCmdMap[] = {
    ...
    {NVTEVT_WIFI_EXE_MOVIE_PARK, WiFiCmd_OnExeMoviePark},
//#NT#2016/03/23#Isiah Chang -begin
//#NT#add new Wi-Fi UI flow.
	{NVTEVT_WIFI_EXE_HEARTBEAT, WiFiCmd_OnExeHB},
	{NVTEVT_WIFI_EXE_APP_SESSION_CLOSE, WiFiCmd_OnAppSessionClose},
	{NVTEVT_WIFI_EXE_TIMELAPSE_TIME, WiFiCmd_OnExeTimeLapseTime},
