...
extern void FlowMovie_SetCurrRecTotalTime(BOOL bAdd);
extern void FlowWifiMovie_IconHideMaxRecTime(VControl *pCtrl);
extern void FlowWifiMovie_IconDrawRecTime(VControl *pCtrl);
...
#endif
	FlowMovie_SetRecCurrTime(0);
	FlowMovie_SetCurrRecTotalTime(FALSE);
...
INT32 WiFiCmd_OnMovieOneSec(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	if (WiFiCmd_GetStatus() == WIFI_MOV_ST_RECORD && paramNum) {
		...
		FlowWifiMovie_IconDrawRecTime(&UIMenuWndWiFiMobileLinkOK_Static_timeCtrl);

		if (UxState_GetData(&UIMenuWndWiFiMobileLinkOK_Status_RECCtrl, STATE_CURITEM) == UIMenuWndWiFiMobileLinkOK_Status_REC_ICON_REC_TRANSPAENT) {
			UxState_SetData(&UIMenuWndWiFiMobileLinkOK_Status_RECCtrl, STATE_CURITEM, UIMenuWndWiFiMobileLinkOK_Status_REC_ICON_REC_ELLIPSE);
		} else {
			UxState_SetData(&UIMenuWndWiFiMobileLinkOK_Status_RECCtrl, STATE_CURITEM, UIMenuWndWiFiMobileLinkOK_Status_REC_ICON_REC_TRANSPAENT);
		}
