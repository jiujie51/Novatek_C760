...
static void PBView_DrawSingleView(PIMG_BUF pSrc, PIMG_BUF pDst, PUSIZE dst_ratio)
{
	...
	PURECT prcVdoWIN = 0;
	CROP_VIDEO_PLAY InitCropVdoPlayInfo;
	USIZE DevRatio = GxVideo_GetDeviceAspect(DOUT1);
	...
	//Only init the crop info for playing video of wide panel. It must call the FlowPB_SetFullCropDisp to decide execution or not.	
	if (bIsMovieFile == TRUE){   
	    InitCropVdoPlayInfo.SrcCropRect.w = pSrc->Width;
		InitCropVdoPlayInfo.SrcCropRect.h = ALIGN_CEIL_4(pSrc->Width*DevRatio.h/DevRatio.w);
		InitCropVdoPlayInfo.SrcCropRect.x = 0; 
		InitCropVdoPlayInfo.SrcCropRect.y = (pSrc->Height - InitCropVdoPlayInfo.SrcCropRect.h)>>1;; 	
		InitCropVdoPlayInfo.VdoOrigSize.w = pSrc->Width;
		InitCropVdoPlayInfo.VdoOrigSize.h = pSrc->Height;
		AppDisp_MoviePlayView_InitSrcVdoCropInfo(&InitCropVdoPlayInfo);	
	}

    // Still video display for wide panel 
    if (FlowPB_GetFullCropDisp() && (bIsMovieFile == TRUE)){
		DstRegion.w = pDst->Width;
		DstRegion.h = pDst->Height;		
		DstRegion.x = 0;
		DstRegion.y = 0;

		SrcRegion->w = pSrc->Width;
		SrcRegion->h = ALIGN_CEIL_4(pSrc->Width*DevRatio.h/DevRatio.w);
		SrcRegion->x = 0;
		SrcRegion->y = (pSrc->Height - SrcRegion->h)>>1;
	}
