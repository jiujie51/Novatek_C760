...
#define MOVIE_RO_FOLDER		 	"EMR_S"   
#define MOVIE_SHARE_FOLDER		"EMR_S" 
#define MOVIE_SHARE_RO_FOLDER	"EMR_S" 
...
static ER xUIPlay_FList_FDB_DelFile(UINT32  cmdID)
{
	...
	if (cmdID == PB_DELETE_ONE) {
		...
		if (FileAttr) {
			...
			debug_msg("delete filepath:%s\r\n",FileAttr->filePath);
			debug_msg("delete file:%s\r\n",FileAttr->filename);
			if (ret == FST_STA_OK) {
				//#NT#2016/10/03#Hideo Lin -begin
				//#NT#To delete both of main and clone movies
#if (SMALL_CLONE_MOVIE == ENABLE)
...
void UIPlay_Delete(UINT32 Command)
{
	...
	//xUIPlay_FList_FDB_DelFile(0);
	} else {
		xUIPlay_FList_DCF_DelFile(Command);
	//xUIPlay_FList_DCF_DelFile(0);
	}
	//FileDB_Refresh(0);
...
/*	UINT32  *pThumbSeqID;
	...
	DBGH(CurrFileSeqID);
	FileSys_DeleteFile(pFileAttr->filePath);
*/
	FileDB_Refresh(0);
	//debug_msg("delete filepath:%s\r\n",pFileAttr->filePath);
	//DBGH(CurrFileSeqID);
