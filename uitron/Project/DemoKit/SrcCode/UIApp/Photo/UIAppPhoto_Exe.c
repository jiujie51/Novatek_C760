...
static UINT32 g_i32PhotoFileSerialNum = 0;
...
#if ((PHOTO_MODE_CAP_YUV420_FUNC == ENABLE) || (_BOARD_DRAM_SIZE_ == 0x04000000))
...
{
	return g_i32PhotoFileSerialNum;
}
...
static void PhotoExe_InitIPL(USIZE *pImageRatioSize)
{
    ...
    for (sensor_index=0;sensor_index<localInfo->sensorCount;sensor_index++) {
		...
		if(localInfo->sensorCount ==1){
			...
			#else
			ImageUnit_SetVdoDirection(ISF_IN1, (UI_GetData(FL_MOVIE_SENSOR_ROTATE) == SEN_ROTATE_ON) ? ISF_VDO_DIR_MIRRORX : ISF_VDO_DIR_NONE);
			#endif
		ImageUnit_End();
...
static void PhotoExe_StartNetworkStream(void)
{
	...
	ImageApp_Photo_UpdateImgLinkForStrm(PHOTO_STRM_ID_1, ENABLE, TRUE);
	#endif
	if (System_GetEnableSensor() == (SENSOR_1 | SENSOR_2)){
	#if !defined(_SENSOR2_CMOS_OFF_)
		ImageApp_Photo_UpdateImgLinkForStrm(PHOTO_STRM_ID_2, ENABLE, TRUE);
	#endif
	}else if (System_GetEnableSensor() == SENSOR_1 ){
	#if !defined(_SENSOR2_CMOS_OFF_)
		ImageApp_Photo_UpdateImgLinkForStrm(PHOTO_STRM_ID_2, DISABLE, TRUE);
	#endif
	}
#else
	#if !defined(_SENSOR1_CMOS_OFF_)
	ImageApp_Photo_UpdateImgLinkForStrm(PHOTO_STRM_ID_1, ENABLE, FALSE);
	#endif
#endif
	ImageApp_Photo_WiFiLinkStart(PHOTO_STRM_ID_1);
	p_strm = UIAppPhoto_get_StreamConfig(UIAPP_PHOTO_STRM_ID_1);
