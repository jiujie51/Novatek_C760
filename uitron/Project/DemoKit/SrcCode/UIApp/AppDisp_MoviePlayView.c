...
static CROP_VIDEO_PLAY  g_CropVdoPlayInfo = {0};//Don't directly modify, please call get AP first to set info and then call set API again.
...
//#NT#2018/09/13#Isiah Chang -begin
//#NT# Use GXIMG_SCALE_BILINEAR to due to IME: path1 does not support ISD. JiraID: NA51023-296.
#if 0//!defined(_MODEL_Q09_2_)
	GxImg_ScaleMultiOut(pSrc, &SrcRect, pMultiDst);
#else

    if (FlowPB_GetFullCropDisp()){
		pMultiDst->pRegion[0]->x = 0;
		pMultiDst->pRegion[0]->y = 0;
		pMultiDst->pRegion[0]->w = pMultiDst->pImg[0]->Width;
		pMultiDst->pRegion[0]->h = pMultiDst->pImg[0]->Height;

		SrcRect.x = g_CropVdoPlayInfo.SrcCropRect.x;
		SrcRect.y = g_CropVdoPlayInfo.SrcCropRect.y;
		SrcRect.w = g_CropVdoPlayInfo.SrcCropRect.w;
		SrcRect.h = g_CropVdoPlayInfo.SrcCropRect.h;	

		//set current crop flag to prev
		if (FlowPB_GetFullCropDisp() != FlowPB_GetFullCropDispPrev()){
			FlowPB_SetFullCropDispPrev(FlowPB_GetFullCropDisp());
		}
    }else{ 
    	#if 0 //mark for p60 fps movie
		//Fill black background
		//for crop play and normal play dynamic change
		hwmem_open();
		hwmem_memset(pMultiDst->pImg[0]->PxlAddr[0], 0x00, pMultiDst->pImg[0]->Height*pMultiDst->pImg[0]->LineOffset[0]);
		hwmem_memset(pMultiDst->pImg[0]->PxlAddr[1], 0x80, pMultiDst->pImg[0]->Height*pMultiDst->pImg[0]->LineOffset[1]/2);
		hwmem_close();
		#endif
    }
...
void AppDisp_MoviePlayView_InitSrcVdoCropInfo(CROP_VIDEO_PLAY *pSrcVdoCropInfo)
{
	g_CropVdoPlayInfo = *pSrcVdoCropInfo; 
}

void AppDisp_MoviePlayView_SetSrcVdoCropInfo(CROP_VIDEO_PLAY *pCropVdoPlayInfo)
{    
    IRECT SrcRect = {0};

	SrcRect = pCropVdoPlayInfo->SrcCropRect;

    if ((SrcRect.x < 0) || (SrcRect.y < 0) || (SrcRect.x != g_CropVdoPlayInfo.SrcCropRect.x) ||
		(SrcRect.y > g_CropVdoPlayInfo.VdoOrigSize.h) || ((SrcRect.y + SrcRect.h) > g_CropVdoPlayInfo.VdoOrigSize.h) ||
		(SrcRect.h != g_CropVdoPlayInfo.SrcCropRect.h)){
		return;
    }
	
	g_CropVdoPlayInfo.SrcCropRect = SrcRect;
}

CROP_VIDEO_PLAY AppDisp_MoviePlayView_GetSrcVdoCropInfo(void)
{
	return g_CropVdoPlayInfo;
}
