...
UINT16* g_pVsFontDataAddr[VIDEO_IN_MAX]={NULL};
#endif
...
#if 0
                   MovieStamp_VsUpdateOsd(i, FALSE, i, 1, 1, g_MovieStampPos[i].uiX, g_MovieStampPos[i].uiY, g_VsOsdWidth[i], g_VsOsdHight[i], (void*)g_pVsFontDataAddr[i]);
#if defined (WATERLOGO_FUNCTION) && (WATERLOGO_FUNCTION == ENABLE)
                   if (g_bWaterLogoEnable[i]){
	                   MovieStamp_VsUpdateOsd(i, FALSE, (16+i), 2, 2, g_MovieWaterInfo[i].uiXPos, g_MovieWaterInfo[i].uiYPos, g_MovieWaterInfo[i].uiWidth, g_MovieWaterInfo[i].uiHeight, (void*)g_MovieWaterInfo[i].uiWaterLogoYAddr);
                   }
#endif
#endif
                   if(g_pVsFontDataAddr[i] && g_VsOsdWidth[i] && g_VsOsdHight[i]) {
		           memset((void*)g_pVsFontDataAddr[i],0, g_VsOsdWidth[i]*g_VsOsdHight[i]);
                   }
		}
#endif
...
void MovieStamp_Setup(UINT32 uiVidEncId, UINT32 uiFlag, UINT32 uiImageWidth, UINT32 uiImageHeight, WATERLOGO_BUFFER *pWaterLogoBuf)
{
...
#if(VIDEOSPRITE_STAMP_FUNC == ENABLE)
		g_VsFontIn[uiVidEncId].pFont=(FONT *)gDateStampFontTbl42x72;
#endif
        break;
	case 2560:  // 2560x1440
	case 2304:  // 2304x1296
	...
	//DBGD(uiIconID);
	//DBGD(pDB->pIconHeader[uiIconID].uiWidth);
	//DBGD(pDB->pIconHeader[uiIconID].uiHeight);
	if(3840 == uiImageWidth)
	{
		pStampInfo->ui32FontWidth  = 42;
		pStampInfo->ui32FontHeight = 72;
