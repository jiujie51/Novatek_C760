...
void OS_Main(void)
{
	MODELEXT_HEADER *header,*header2;
	DRAM_PARTITION *p_dram_partition = (DRAM_PARTITION *)Dx_GetModelExtCfg(MODELEXT_TYPE_DRAM_PARTITION, &header2);
...
#if (LOGFILE_FUNC==ENABLE)
	{
		...
		cfg.LogBuffSize2 = sizeof(gLogFile_Buff2);
		#endif
		LogFile_Config(&cfg);
	}
