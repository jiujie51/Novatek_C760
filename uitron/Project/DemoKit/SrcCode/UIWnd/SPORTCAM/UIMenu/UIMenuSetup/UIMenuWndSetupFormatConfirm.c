	...
	if (UxList_GetData(pCtrl, LST_CURITM) == UIMenuWndSetupFormatConfirm_List_Text_STRID_OK)
	{
		...
		switch(uiKeyAct)
		{
			 case NVTEVT_KEY_PRESS:
				if(g_u32FormatIndex==0)
				{
					Ux_OpenWindow(&UIFlowWndWaitMomentCtrl,1,0);
					BKG_PostEvent(NVTEVT_BKW_FORMAT_NAND);
				}
				else if(g_u32FormatIndex==1)
				{
					...
					if(System_GetState(SYS_STATE_CURRSUBMODE)==SYS_SUBMODE_WIFI)
					{
						...
						{
							Ux_SendEvent(0,NVTEVT_WIFI_EXE_MOVIE_REC, 1, 0);
						}
					}
					else
					{
						...
						{
							debug_msg(" If is recording,must stop Rec !!!![%06d], %s\r\n", __LINE__, __func__);
							FlowMovie_StopRec();
							SwTimer_DelayMs(500);
						}
					}
					if((FlowMovie_GetMovDataState() != MOV_ST_REC)||(WiFiCmd_GetStatus() != WIFI_MOV_ST_RECORD))
					{
						Ux_OpenWindow(&UIFlowWndWaitMomentCtrl,1,0);
						BKG_PostEvent(NVTEVT_BKW_FORMAT_CARD);
					}
