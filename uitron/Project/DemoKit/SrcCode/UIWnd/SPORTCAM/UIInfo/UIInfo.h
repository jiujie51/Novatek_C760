...
typedef struct _UIMenuUIMenuStoreInfo {
	...
	// RTC time 
	struct tm DateTimeStored;

	// Format SD Remind
	BOOL   bFormatRemind;

	BOOL   bTimelapsereboot;
    //check sum	
	UINT32 uiMenuStoreInfoCheckSum;
} UIMenuStoreInfo;

typedef struct _UISysSN
{
	CHAR   strSN[SERIAL_NUM_LEN]; 
	UINT32   uidChksum;
}UISysSN;
...
typedef enum {
	...
	FL_LowBatteryLvl,				//low batter level: 12v/ 11.8v/ 11.6v
	FL_movie_timelapse_set,			//timelapse set: off/12h/24h/48h
	FL_MOVIE_TIMELAPSE_TIME,
	//Info Status
	FL_LensOpen,
	FL_TakePictCnt,
	FL_IsDCIMReadOnly,
...
enum _MOVIE_MENU_SIZE {
	...
	MOVIE_SIZE_DUAL_2560x1440P30_2560x1440P30,		/// QHD QHD
	MOVIE_SIZE_DUAL_2560x1440P30_1920x1080P30,       ///MT pqw add:1440p+1080p
	...
	MOVIE_SIZE_DUAL_1280x720P30_1920x1080P30,		/// MT pqw add:720p+1080p
...
// Openning Audio
//-----------------------------------------------------------------------------
enum _OPENNING_AUDIO {
	OPENNING_AUDIO_ON,
	OPENNING_AUDIO_OFF,
	OPENNING_AUDIO_ID_MAX
};
...
// low battery Level
//-----------------------------------------------------------------------------
enum _LOW_BATTERY_LEVEL {
	LOWBATTERY_LVL_1 = 0, //12v
	LOWBATTERY_LVL_2,	  //11.8v 
	LOWBATTERY_LVL_3,     //11.6v
	LOWBATTERY_LVL_ID_MAX
};

//-----------------------------------------------------------------------------
// Movie time lapse set
//-----------------------------------------------------------------------------
enum _MOVIE_TIMELAPSE_SET
{
    MOVIE_TIMELAPSE_SET_OFF = 0,
    MOVIE_TIMELAPSE_SET_12HOUR,
    MOVIE_TIMELAPSE_SET_24HOUR,
    MOVIE_TIMELAPSE_SET_48HOUR,
    MOVIE_TIMELAPSE_SET_ID_MAX
};
...
extern void     Reset_MenuInfo_AfUploadFW(void);
...
extern void UI_SetSNInfo(char *buff);
extern char * UI_GetSNInfo(void);
extern void Load_SysSNInfo(void);
extern void Save_SysSNInfo(void);
extern 	BOOL Load_MenuInfoForDisp(void);
extern void System_SaveDateTime(void);
extern void System_LoadStoredDateTime(void);
extern BOOL System_IsLoadDateTimeNeeded(void);
BOOL Get_TimelapsereRebootStatus(void );
void Set_TimelapsereRebootStatus(BOOL        Timelapsestatus);
extern 	void System_UpdateFWReboot(void);
