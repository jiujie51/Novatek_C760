...
void SysResetSsidPsw(void)
{
   CHKPNT;
   memset((void *)(&currentInfo.strSSID), 0, sizeof(currentInfo.strSSID));
   memset((void *)(&currentInfo.strPASSPHRASE), 0, sizeof(currentInfo.strPASSPHRASE));
}
...
{	
	struct tm cUIDateTime;    //load date-time   
	...
	if ( (rtc_isPowerLost() || cUIDateTime.tm_year < MIN_YEAR ))	//RTC if 2000 change to Default..   
	{        
...
void SysCheckFlag(void)
{
	...
	SysLimitFlag(FL_MOVIE_MOTION_DET_MENU,   0,  MOVIE_MOTIONDET_ID_MAX,     DEFAULT_MOVIE_MOTION_DET);
    ...
    SysLimitFlag(FL_MOVIE_PARK,          0,  MOVIE_PARK_ID_MAX,           DEFAULT_MOVIE_PARK);
 	SysLimitFlag(FL_LowBatteryLvl,       0,  LOWBATTERY_LVL_ID_MAX,    	  DEFAULT_BATTERY_LVL);
	SysLimitFlag(FL_movie_timelapse_set, 0,  MOVIE_TIMELAPSE_SET_ID_MAX,  DEFAULT_TIME_LAPSE_SET);
...
void SysSetFixedFlagSysInit(void)
{
	...
	SysSetFlag(FL_MOVIE_DATEIMPRINT,SysGetFlag(FL_MOVIE_DATEIMPRINT_MENU));//zjf
...
void SysResetFlag(void)
{
	...
	SysSetFlag(FL_MOVIE_MOTION_DET_MENU,     DEFAULT_MOVIE_MOTION_DET);
	SysSetFlag(FL_MOVIE_AUDIO,          DEFAULT_MOVIE_AUDIO);
	SysSetFlag(FL_MOVIE_DATEIMPRINT,    DEFAULT_MOVIE_DATEIMPRINT);
	SysSetFlag(FL_MOVIE_DATEIMPRINT_MENU,    DEFAULT_MOVIE_DATEIMPRINT);//zjf 20201010
	...
	SysSetFlag(FL_LowBatteryLvl,               DEFAULT_BATTERY_LVL);
	SysSetFlag(FL_movie_timelapse_set,         DEFAULT_TIME_LAPSE_SET);
	...
	SysSetFlag(FL_WIFI, DEFAULT_FL_WIFI);
...
BOOL Get_TimelapsereRebootStatus(void )
{
	return currentInfo.bTimelapsereboot;
}

void Set_TimelapsereRebootStatus(BOOL Timelapsestatus )
{
	currentInfo.bTimelapsereboot = Timelapsestatus;
}
...
void System_SaveDateTime(void)
{
  currentInfo.DateTimeStored = HwClock_GetTime(TIME_ID_CURRENT);
  debug_msg("Liwk ------ SaveDate Time :%d/%d/%d %d:%d:%d\r\n",currentInfo.DateTimeStored.tm_year, currentInfo.DateTimeStored.tm_mon, currentInfo.DateTimeStored.tm_mday, currentInfo.DateTimeStored.tm_hour, currentInfo.DateTimeStored.tm_min, currentInfo.DateTimeStored.tm_sec);
  Save_MenuInfo();
}
void System_LoadStoredDateTime(void)
{
     HwClock_SetTime(TIME_ID_CURRENT, currentInfo.DateTimeStored, 0);
}

BOOL System_IsLoadDateTimeNeeded(void)
{
   struct tm DateTimeCur;
   DateTimeCur = HwClock_GetTime(TIME_ID_CURRENT);
   debug_msg("\r\n mmmm store year:%d-month:%d\r\n",currentInfo.DateTimeStored.tm_year,currentInfo.DateTimeStored.tm_mon);
   if(DateTimeCur.tm_year <= DEF_YEAR && DateTimeCur.tm_mon== DEF_MONTH && DateTimeCur.tm_mday == DEF_DAY &&\
   	  DateTimeCur.tm_hour== 0 && DateTimeCur.tm_min == 0)
   {
      if(currentInfo.DateTimeStored.tm_year < MAX_YEAR && currentInfo.DateTimeStored.tm_year >= MIN_YEAR)
      {
          return TRUE;
      }
   }
...
void System_UpdateFWReboot(void)
{
   ...
   // CHKPNT;    
    ...
    if(uiFWUpdate)
    {
     //  CHKPNT;
	   System_WaitFS();
       result = FileSys_DeleteFile("A:\\FW96670A.bin");
	   DBGD(result);
