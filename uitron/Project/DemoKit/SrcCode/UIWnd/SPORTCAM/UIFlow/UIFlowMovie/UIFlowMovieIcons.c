...
static CHAR    *resolution_Buf[MOVIE_SIZE_ID_MAX] = {
	...
	[MOVIE_SIZE_FRONT_3840x2160P30] = "2160P+1080P",//"UHD P30",
	...
	[MOVIE_SIZE_FRONT_2560x1440P30] = "1440P+1080P",//"QHD P30",
	...
	[MOVIE_SIZE_FRONT_1920x1080P30] = "1080P+1080P",//"FHD P30",
...
void FlowMovie_IconDrawMicroPhone(VControl *pCtrl)
{
    UxState_SetData(pCtrl,STATE_CURITEM,SysGetFlag(FL_MOVIE_AUDIO));
    UxCtrl_SetShow(pCtrl,TRUE);
}

void FlowMovie_IconDrawPark(VControl *pCtrl)
{
	if(SysGetFlag(FL_MOVIE_PARK) == MOVIE_PARK_OFF)
    	UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_Parking_ICON_BSD_OFF);
	else
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_Parking_ICON_BSD_ON);

	UxCtrl_SetShow(pCtrl, TRUE);	   
}

void FlowMovie_IconHidePark(VControl *pCtrl)
{
    UxCtrl_SetShow(pCtrl, FALSE);
...
void FlowMovie_IconDrawTimelapse(VControl *pCtrl)
{
	...
	else if(SysGetFlag(FL_movie_timelapse_set) == MOVIE_TIMELAPSE_SET_12HOUR)
	{CHKPNT;
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_TimeLapse_ICON_CYCLIC_REC_1MIN);
		UxCtrl_SetShow(pCtrl, TRUE);
	}
	else if(SysGetFlag(FL_movie_timelapse_set) == MOVIE_TIMELAPSE_SET_24HOUR)
	{CHKPNT;
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_TimeLapse_ICON_CYCLIC_REC_2MIN);
		UxCtrl_SetShow(pCtrl, TRUE);
	}
	else if(SysGetFlag(FL_movie_timelapse_set) == MOVIE_TIMELAPSE_SET_48HOUR)
	{CHKPNT;
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_TimeLapse_ICON_CYCLIC_REC_3MIN);
		UxCtrl_SetShow(pCtrl, TRUE);
	}*/
...
void FlowMovie_IconDrawSnap(VControl *pCtrl)
{

    /*UxState_SetData(pCtrl,STATE_CURITEM,1); 
    UxCtrl_SetShow(pCtrl, TRUE);*/
    if(SysGetFlag(FL_movie_timelapse_set) == MOVIE_TIMELAPSE_SET_OFF)
	{
    	UxCtrl_SetShow(pCtrl, FALSE);
	...
	{
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_Snap_ICON_P12H);
		UxCtrl_SetShow(pCtrl, TRUE);
	}
	else if(SysGetFlag(FL_movie_timelapse_set) == MOVIE_TIMELAPSE_SET_24HOUR)
	{
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_Snap_ICON_P24H);
		UxCtrl_SetShow(pCtrl, TRUE);
	}
	else if(SysGetFlag(FL_movie_timelapse_set) == MOVIE_TIMELAPSE_SET_48HOUR)
	{
	    UxState_SetData(pCtrl,STATE_CURITEM, UIFlowWndMovie_Status_Snap_ICON_P48H);
		UxCtrl_SetShow(pCtrl, TRUE);
	}
    

}
void FlowMovie_IconHideSnap(VControl *pCtrl)
{
    UxCtrl_SetShow(pCtrl, FALSE);

}

void FlowMovie_IconHideMicroPhone(VControl *pCtrl)
{
    UxCtrl_SetShow(pCtrl,FALSE);
}
...
void FlowMovie_IconDrawGPS(VControl *pCtrl,BOOL GPSUpdate)
{
#if(GPS_FUNCTION==ENABLE)
    static BOOL pregpsstatus = FALSE;
    static BOOL curgpsstatus = FALSE;
    //static int gps_link_cnt = 0;
	if (GPSUpdate)
	{
		pregpsstatus = FALSE;
		curgpsstatus = FALSE;
	...
	//gps状态有变化,给出信息和语音提示
	if(pregpsstatus != curgpsstatus)
	{		
		if(curgpsstatus == TRUE)
		{
			UxState_SetData(pCtrl,STATE_CURITEM,UIFlowWndMovie_Status_GPS_ICON_LOCATION);
		}
		else
		{		
			UxState_SetData(pCtrl,STATE_CURITEM,UIFlowWndMovie_Status_GPS_ICONID_NULL);
		}
		pregpsstatus = curgpsstatus;
	}
	UxCtrl_SetShow(pCtrl,TRUE);
#endif
...
void FlowMovie_IconDrawWifi(VControl *pCtrl)
{
	if(!bEnterTimelapse)
	{
		if((UI_GetData(FL_WIFI_LINK) == WIFI_LINK_OK)  ||(UI_GetData(FL_WIFI_LINK) == WIFI_LINK_LINKING) )   
		    UxState_SetData(pCtrl, STATE_CURITEM, UIFlowWndMovie_Status_WIFI_ICON_WIFI_ON);
		else
		    UxState_SetData(pCtrl, STATE_CURITEM, UIFlowWndMovie_Status_WIFI_ICON_WIFI_OFF);
...
BOOL g_wifi_info_show = TRUE;
void FlowMovie_DrawSSIDPassWord(void)
{
	...
	if (UI_GetData(FL_WIFI_LINK) == WIFI_LINK_OK) {
		if (UI_GetData(FL_NetWorkMode) == NET_AP_MODE || UI_GetData(FL_NetWorkMode) == NET_WPS_AP_PBC_MODE || UI_GetData(FL_NetWorkMode) == NET_STATION_MODE) {
			...
			UxStatic_SetData(&UIFlowWndMovie_StaticTXT_SSIDCtrl, STATIC_VALUE, Txt_Pointer(buf1));
			UxStatic_SetData(&UIFlowWndMovie_StaticTXT_KeyCtrl, STATIC_VALUE, Txt_Pointer(buf2));
		}
	} else {
		snprintf(buf1, sizeof(buf1), "%s:Off", "Conntecting");
		UxStatic_SetData(&UIFlowWndMovie_StaticTXT_SSIDCtrl, STATIC_VALUE, Txt_Pointer(buf1));
		UxStatic_SetData(&UIFlowWndMovie_StaticTXT_KeyCtrl, STATIC_VALUE, STRID_NULL);
	}
	if(g_wifi_info_show)
	{
		UxCtrl_SetShow(&UIFlowWndMovie_StaticTXT_SSIDCtrl, TRUE);
		UxCtrl_SetShow(&UIFlowWndMovie_StaticTXT_KeyCtrl, TRUE);
	}
	else
	{
		UxCtrl_SetShow(&UIFlowWndMovie_StaticTXT_SSIDCtrl, FALSE);
		UxCtrl_SetShow(&UIFlowWndMovie_StaticTXT_KeyCtrl, FALSE);
	}
		

}
void FlowMovie_DrawLock(BOOL bDraw)
{
   if(bDraw)
   {
       UxState_SetData(&UIFlowWndMovie_Status_LockCtrl, STATE_CURITEM, 1); 	   
       UxCtrl_SetShow(&UIFlowWndMovie_Status_LockCtrl, bDraw);
   }
   else
   {
       UxState_SetData(&UIFlowWndMovie_Status_LockCtrl, STATE_CURITEM, 0); 	   
       UxCtrl_SetShow(&UIFlowWndMovie_Status_LockCtrl, bDraw);      
   }
}

void FlowMovie_DrawCardFreeCapacity(void)
{	
	static CHAR	buf[8];
	
	snprintf(buf, sizeof(buf), "%   lld  GB",(FileSys_GetDiskInfo(FST_INFO_FREE_SPACE) / 1024/1024/1024));
	//debug_msg("\r\n 1[rec]Free Space = %lld \r\n", FileSys_GetDiskInfo(FST_INFO_FREE_SPACE));
	//debug_msg("\r\n 2[rec]Free Space = %lld KB\r\n", FileSys_GetDiskInfo(FST_INFO_FREE_SPACE)/1024);
	//debug_msg("\r\n 3[rec]Free Space = %lld MB\r\n", FileSys_GetDiskInfo(FST_INFO_FREE_SPACE)/1024/1024);
	//debug_msg("\r\n 4[rec]Free Space = %lld GB\r\n", FileSys_GetDiskInfo(FST_INFO_FREE_SPACE)/1024/1024/1024);

	
	UxStatic_SetData(&UIFlowWndMovie_Static_CardFreeCapacityCtrl, STATIC_VALUE, Txt_Pointer(buf)); 	   
	UxCtrl_SetShow(&UIFlowWndMovie_Static_CardFreeCapacityCtrl, TRUE);
}
...
void FlowMovie_UpdateIcons(BOOL bShow)
{
	if (bShow == FALSE || SysGetFlag(FL_LCD_DISPLAY) == DISPOUT_OFF) {
		...
		FlowMovie_DrawPIM(FALSE);
		FlowMovie_IconHideMicroPhone(&UIFlowWndMovie_Status_MicroPhoneCtrl);
	    FlowMovie_IconHidePark(&UIFlowWndMovie_Status_ParkingCtrl);
	    FlowMovie_IconHideSnap(&UIFlowWndMovie_Status_SnapCtrl);
	    FlowMovie_IconHideGsensor(&UIFlowWndMovie_Status_GsensorCtrl);
		...
		FlowMovie_IconDrawPark(&UIFlowWndMovie_Status_ParkingCtrl);	
		FlowMovie_IconDrawSnap(&UIFlowWndMovie_Status_SnapCtrl);
