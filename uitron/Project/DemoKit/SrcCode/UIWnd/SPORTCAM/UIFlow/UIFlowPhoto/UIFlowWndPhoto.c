...
//#MT#lyb -20191212 -begin
static UINT32  g_uiPhotoDoubleClickTimerID = NULL_TIMER;
//#MT#lyb -20191212 -end
...
INT32 UIFlowWndPhoto_OnKeyCapture(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_KEY_CAPTURE, UIFlowWndPhoto_OnKeyCapture)
	...
	case NVTEVT_KEY_PRESS: {
      ...
      //#NT#2018/12/13#hilex Lin - end
				...
				default:
					...
					//pqw del because of black flash show//UI_Show(UI_SHOW_PREVIEW, FALSE);
					...
					UISound_Play(DEMOSOUND_SOUND_SHUTTER_TONE);					break;
				}
				break;
...
//#MT#lyb -20191212 -begin
    if (g_uiPhotoDoubleClickTimerID == NULL_TIMER) {
        g_uiPhotoDoubleClickTimerID = GxTimer_StartTimer(TIMER_DOUBLE_CLICK, NVTEVT_DOUBLECLICK_TIMER, CONTINUE);
    }
//#MT#lyb -20191212 -end
...
INT32 UIFlowWndPhoto_OnClose(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	if (g_uiPhotoDoubleClickTimerID != NULL_TIMER) {
		GxTimer_StopTimer(&g_uiPhotoDoubleClickTimerID);
	}
...
//ÖÇÄÜÓïÒôÅÄÕÕ´¦Àí
INT32 UIFlowWndPhoto_OnKeyCapture(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    UISound_Play(DEMOSOUND_SOUND_SHUTTER_TONE);
	return UIFlowWndPhoto_OnExeCapture(pCtrl, paramNum, paramArray);
}

static UINT8 uiPhotoDoubleClickCnt_MenuKey = 0;
INT32 UIFlowWndPhoto_OnKeyMenu(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
#if 1
    UINT32  uiKeyAct, uiKeyDouble;
    UINT32  uiSoundMask;

    uiKeyAct = paramArray[0];
    uiKeyDouble = paramArray[1];
   //TESTLOG("%d, %d", paramNum, uiKeyDouble);

    switch (uiKeyAct) {
        case NVTEVT_KEY_PRESS:
            if(paramNum == 1)
            {
                uiPhotoDoubleClickCnt_MenuKey++;
            }
            else if(paramNum == 2)
            {
                if(1)//(uiKeyDouble == DoubleClick_ONCE) 
                {
...
//#MT#lyb -20191212 -begin
    case NVTEVT_DOUBLECLICK_TIMER://100ms
        {
        static UINT8 uiDoubleCLick_TimerMenuCnt = 0;

        if(uiPhotoDoubleClickCnt_MenuKey > 0)
        {
            uiDoubleCLick_TimerMenuCnt++;
            if(uiDoubleCLick_TimerMenuCnt > 4)
            {
                if(uiPhotoDoubleClickCnt_MenuKey >= 2){
                    //TESTLOG("double click");
                    //Ux_PostEvent(NVTEVT_KEY_MENU, 2, NVTEVT_KEY_PRESS, DoubleClick_DOUBLE);

                } else {
                    //TESTLOG("one click");
                    //Ux_PostEvent(NVTEVT_KEY_MENU, 2, NVTEVT_KEY_PRESS, DoubleClick_ONCE);
                }
                uiPhotoDoubleClickCnt_MenuKey = 0;
                uiDoubleCLick_TimerMenuCnt = 0;
            }
        }
        }
        break;
