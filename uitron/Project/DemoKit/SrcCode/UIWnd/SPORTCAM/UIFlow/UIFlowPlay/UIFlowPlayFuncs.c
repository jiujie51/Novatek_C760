...
static BOOL g_bIsFullCropDisp = FALSE;
static BOOL g_bIsFullCropDispPrev = FALSE;
...
PURECT FlowPB_ThumbDisplay(void)
{
	...
	if (KeyScan_GetPlugDev() == PLUG_TV) {
		...
		if (KeyScan_GetPlugDev() == PLUG_HDMI) {
			...
			//LCD
#if (FLOWPB_THUMB_MODE == FLOWPB_THUMB_MODE_3_3)
			ThumbCfg.wFirstHGap         = 36;//18;//16;
			ThumbCfg.wHGap              = 34;//17;//16;
			ThumbCfg.wLastHGap          = 36;//18;//32;
			ThumbCfg.wFirstVGap         = 12;//6;
			ThumbCfg.wVGap              = 12;//6;
			ThumbCfg.wLastVGap          = 60;//30;
#elif (FLOWPB_THUMB_MODE == FLOWPB_THUMB_MODE_3_2)
...
void FlowPB_SetFullCropDisp(BOOL bCropDispFlag)
{
	g_bIsFullCropDisp = bCropDispFlag;
}

BOOL FlowPB_GetFullCropDisp(void)
{
	return g_bIsFullCropDisp;
}

void FlowPB_SetFullCropDispPrev(BOOL bCropDispFlag)
{
	g_bIsFullCropDispPrev = bCropDispFlag;
}

BOOL FlowPB_GetFullCropDispPrev(void)
{
	return g_bIsFullCropDispPrev;
}
