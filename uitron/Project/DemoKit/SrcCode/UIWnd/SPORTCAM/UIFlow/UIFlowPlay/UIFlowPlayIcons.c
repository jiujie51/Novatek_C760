...
static void FlowPB_IconDrawThumbSeq(BOOL bShow)
{
	...
	//PB_GetParam(PBPRMID_CURR_FILEPATH,(UINT32 *)Filename);
	//debug_msg("current filepath:%s \r\n",Filename);
	snprintf(item1_Buf, 32, "%lu/%lu", *(pThumbSeqID + uiBrowseIdx - 1), uiFileNum);
	//snprintf(item1_Buf, 32, "%lu/%lu", index, uiFileNum);
...
void FlowPlayThumb_DrawDateTime(void)
{
	...
	if (uiFileNum)
	{
		...
		pFileAttr = FileDB_SearhFile(0,(CurrFileSeqID-1));
...
void FlowPB_IconDrawMovPlayVolumn(VControl *pCtrl)
{
	UINT32 Data;
	...
	debug_msg("ydw Data  = %d\r\n",Data);
    UxState_SetData(pCtrl,STATE_CURITEM,Data);
    UxCtrl_SetShow(pCtrl, TRUE);
}

void FlowPB_IconHideMovPlayVolumn(VControl *pCtrl)
{
...
void FlowPB_UpdateIcons(BOOL bShow)
{
	if (bShow == FALSE) {
		...
		FlowPB_IconDrawTabNavi(FALSE);
            FlowPB_IconHideMovPlayVolumn(&UIFlowWndPlay_StatusICN_VolumeCtrl);
		...
		if (uiCurrMode == PLAYMODE_PRIMARY) {
            ...
            FlowPB_IconDrawMovPlayVolumn(&UIFlowWndPlay_StatusICN_VolumeCtrl);
...
void FlowPB_ClearAllThumbIcon(void)
{
	...
	UxCtrl_SetShow(&UIFlowWndPlayThumb_ThumbIDCtrl, FALSE);
//	UxCtrl_SetShow(&UIFlowWndPlayThumb_StaticTXT_FilenameCtrl, FALSE);
}
