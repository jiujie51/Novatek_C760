...
#define PLAY_KEY_RELEASE_MASK      FLGKEY_MENU//FLGKEY_KEY_MASK_NULL//(FLGKEY_UP | FLGKEY_DOWN | FLGKEY_LEFT | FLGKEY_RIGHT)
#define PLAY_KEY_CONTINUE_MASK     FLGKEY_MENU//FLGKEY_KEY_CONT_MASK_DEFAULT
...
CTRL_LIST_ITEM(UIFlowWndPlay_StatusICN_Volume)
...
INT32 UIMenuWndPlay_OnDeAuthenticated(VControl *, UINT32, UINT32 *);
...
EVENT_ITEM(NVTEVT_WIFI_DEAUTHENTICATED, UIMenuWndPlay_OnDeAuthenticated)
...
//    PB_OpenSpecFileBySeq(DCF_GetDBInfo(DCF_INFO_TOL_FILE_COUNT), TRUE);
	...
	Ux_OpenWindow(&UIFlowWndPlayThumbCtrl, 0);
	Ux_DefaultEvent(pCtrl, NVTEVT_OPEN_WINDOW, paramNum, paramArray);
	return NVTEVT_CONSUME;
}
...
INT32 UIFlowWndPlay_OnKeySelect(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	/*	Tabfocus = UxTab_GetData(&UIFlowWndPlay_TabNaviCtrl, TAB_FOCUS);
...
#if (_BOARD_DRAM_SIZE_ == 0x04000000)
				// 64MB DRAM cannot support step by step function, since buffer is not enough
		...
		case UIFlowWndPlay_TabNavi_ButtonDown:
			...
			Input_SetKeyMask(KEY_RELEASE, FLGKEY_KEY_MASK_NULL); //FLGKEY_KEY_MASK_DEFAULT			
			break;
		default:
			break;
		}*/
   ...
   case PLB_ST_PAUSE_MOV:		
	...
	switch (uiKeyAct) {
		case NVTEVT_KEY_RELEASE:
			if(curmode != 2)//MODE_PLAYBACK)
			{	
				curmode = 2;//MODE_PLAYBACK;
				debug_msg("\r\n^curmode:%d",curmode);
			...
			else if(g_PlbData.State == PLB_ST_PLAY_MOV)
			{
	...
	switch (uiKeyAct) {
		...
		case NVTEVT_KEY_RELEASE:
			CHKPNT;
			Input_SetKeyMask(KEY_RELEASE, FLGKEY_KEY_MASK_NULL); //FLGKEY_KEY_MASK_DEFAULT	
			Input_SetKeyMask(KEY_CONTINUE, FLGKEY_KEY_MASK_NULL); //FLGKEY_KEY_MASK_DEFAULT	
...
{CHKPNT;
	...
	case PLB_ST_FULL:
			...
			if (uiPBFileFmt & (PBFMT_MOVMJPG | PBFMT_AVI | PBFMT_MP4 | PBFMT_TS)) {
				...
				SMPMaxMemInfo.TotalSecs = VidInfo.uiToltalSecs;
				SMPMaxMemInfo.CodecType = VidInfo.uiVidType;
				debug_msg("VidInfo.uiVidType is %d \r\n",VidInfo.uiVidType);
				debug_msg("VidInfo.uiVidWidth is %d \r\n",VidInfo.uiVidWidth);
				debug_msg("VidInfo.uiVidHeight is %d \r\n",VidInfo.uiVidHeight);
				SMPMaxMemInfo.ImageWidth = VidInfo.uiVidWidth;
				SMPMaxMemInfo.ImageHeight = VidInfo.uiVidHeight;
				gMovie_Play_Info.pmax_mem_info = &SMPMaxMemInfo;
				CHKPNT;
...
CHKPNT;
                ...
                // For step play func. Should be GOP num. of video file. default is 15.
                //ImageApp_MoviePlay_SetParam(MOVIEPLAY_NM_PARAM_RAW_BLK_NUM, 15);
				ImageApp_MoviePlay_SetParam(MOVIEPLAY_NM_PARAM_RAW_BLK_NUM, 4);//pqw
#endif
...
INT32 UIMenuWndPlay_OnDeAuthenticated(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
...
//----------------------UIFlowWndPlay_StatusICN_VolumeCtrl Event---------------------------
EVENT_BEGIN(UIFlowWndPlay_StatusICN_Volume)
EVENT_END
