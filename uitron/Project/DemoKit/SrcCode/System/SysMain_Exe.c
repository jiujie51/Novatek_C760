...
extern void System_WifiStart(void);
...
extern void System_OnStrg_ChkDemo(void);
extern void System_OnStrg_ChkDisp(void);
INT32 System_OnBoot(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
	...
	if (paramNum == 1 && paramArray[0] == 1) { //power-on end
...
#if 1//(POWERONSOUND_FUNCTION == ENABLE)
		...
		GxLED_SetCtrl(KEYSCAN_LED_RED, SET_TOGGLE_LED, TRUE);
         ...
         System_OnStrg_ChkDemo();
         System_OnStrg_ChkDisp();
#endif
...
//	    Ux_CloseWindow(&UIMenuWndWiFiMobileLinkOKCtrl, 0);
    ...
    {
	    if(bAccPowerOff && (SysGetFlag(FL_MOVIE_PARK) != MOVIE_PARK_OFF))
	    {
	        GSensor_SetMovieParkSensitivity(SysGetFlag(FL_MOVIE_PARK));
