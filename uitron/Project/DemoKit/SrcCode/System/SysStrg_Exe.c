...
void System_OnStrgInit_FS(void)
{
    ...
    GxStrg_SetConfigEx(0, FILE_CFG_SUPPORT_EXFAT, FileSys_GetOPS_uITRON());//support exfat
#endif
...
// Detect whether test mode Or not?
 BOOL bDemoStart = FALSE;
 BOOL bDispInfoOnly = FALSE;
 void System_OnStrg_ChkDemo(void)
{
	bDemoStart = TRUE;//factory test mode
	return ;
 ...
 //Detect whether entering  information display mode 
void System_OnStrg_ChkDisp(void)
{
	bDispInfoOnly= TRUE;
	return ;

	CHAR      path[50]="A:\\DispInfo";
    UINT32    index;
	   FST_FILE filehdl = NULL;
	//Wait FileSys command finish.
	index = FileSys_WaitFinish();
