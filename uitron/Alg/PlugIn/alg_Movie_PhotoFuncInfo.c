...
void Movie_md_process(MEM_RANGE *buf, MEM_RANGE *cachebuf)
{
	...
	IMG_BUF *pBuf = NULL;
	#endif
    #if (SENSOR_CAPS_COUNT == 2 && defined(_NVT_ETHREARCAM_NONE_))
	ISF_DATA isf_data;
	ISF_UNIT *pUnit;
	ISF_PORT *pSrc;
	IMG_BUF *pBuf = NULL;		
	...
	if (UI_GetData(FL_MOVIE_MOTION_DET)) {
        ...
        #elif ((SENSOR_CAPS_COUNT == 2) && defined(_NVT_ETHREARCAM_NONE_))
		curr_proc_time = clock();
		if ((curr_proc_time - prev_proc_time) >= proc_period) {
			pUnit = &ISF_UserProc;
			pSrc = ImageUnit_Out(pUnit, ImageApp_MovieMulti_GetAlgDataPort(_CFG_REC_ID_1, _CFG_ALG_PATH3));
			if (ImageUnit_IsAllowPull(pSrc) && ImageUnit_PullData(pSrc, (ISF_DATA *)&isf_data, 0) == ISF_OK) {
				pBuf = (IMG_BUF *)&isf_data.Desc[0];
				md_result1 = MD_Process(0, pBuf);
				prev_proc_time = curr_proc_time;
				ImageUnit_ReleaseData((ISF_DATA *)&isf_data);
			}
			pUnit2 = &ISF_UserProc;
			pSrc2 = ImageUnit_Out(pUnit2, ImageApp_MovieMulti_GetAlgDataPort(_CFG_REC_ID_2, _CFG_ALG_PATH3));
			if (ImageUnit_IsAllowPull(pSrc2) && ImageUnit_PullData(pSrc2, (ISF_DATA *)&isf_data2, 0) == ISF_OK) {
				pBuf2 = (IMG_BUF *)&isf_data2.Desc[0];
				md_result2 = MD_Process(1, pBuf2);
				prev_proc_time = curr_proc_time;
...
static void Movie_LCAProcess(MEM_RANGE *buf, MEM_RANGE *cachebuf)
{
	...
	debug_msg("iVehicleNum=%d\r\n", pLcaVehicleRlt->iVehicleNum);*/
