...
/**
	AD(analog decoder) signal
*/
typedef enum {
	SENSOR_AD_SIGNAL_PARA_SINGLE = 0, 	// parallel output single signal
	SENSOR_AD_SIGNAL_PARA_MUX, 			// parallel output mux signal
	SENSOR_AD_SIGNAL_MIPI_SINGLE, 		// mipi output single signal
	SENSOR_AD_SIGNAL_MIPI_VIR_ID, 		// mipi virtual id
} SENSOR_AD_SIGNAL;
...
typedef struct {
	...
	UINT32 time_out;                ///< wait fmd timeout, Unit in ms. 0 = no timeout, only support CSI and LVDS sensor
									///< default is 1000ms, if you want to manual setting, pls enable sensor_ssenif_timeout_manual(SENSOR_ID id, BOOL en);
...
typedef struct {
	UINT8 ad_idx;		// AD index [range : 0,1,2,3....0xff], start from 0
	UINT8 input_port;	// AD input port [range : 0,1,2,3....0xff], start from 0
	UINT8 output_port;	// AD output port [range : 0,1,2,3....0xff], start from 0
	UINT8 signal;     	// SENSOR_AD_SIGNAL
	UINT8 comb_num; 	// combine id number, how many idx have the ad_idx/output_port/signal
	UINT8 comb_map; 	// combine id map, which input_port mapping (bit mapping) to the output_port
	UINT8 dl_num;		// only mipi AD(analog decoder) need to set. serial interface datalane nunber.
} SENSOR_AD_DRV_INFO;
...
typedef struct {
	...
	SENSOR_AD_DRV_INFO ad_info;		// AD(analog decoder) need to reference
} SEN_DRV_INIT_OBJ;
...
typedef struct {
	...
	SENSOR_MODE_INFO *(*fp_get_sensor_mode_info)(SENSOR_MODE mode); // if <ad condition>, will input SENSOR_ID instead of SENSOR_MODE
	// <ad condition> : "SENSOR_TYPE_AD set in sendrv" & "SENSOR_GP_X set corresponding id in project"
...
typedef struct {
	UINT8 input_port;	// AD input port [range : 0,1,2,3....0xff], pls start from 0
	UINT8 output_port;	// AD output port [range : 0,1,2,3....0xff], pls start from 0
	UINT8 signal;  		// SENSOR_AD_SIGNAL
} SENSOR_AD_EXT_INFO;
...
extern void sensor_set_group_rec(SENSOR_MULTI_ID* sensor_gp_id, SENSOR_MULTI_ID* sensor_subgp_id, UINT32* senname_idx);
extern void sensor_set_ad_ext_info(SENSOR_AD_EXT_INFO* user_ad_ext_info);
...
// serial sensor timeout set by user, if not enable, default is 1000ms (and force disable)
// please set in SENSOR_INIT_OBJ / time_out
extern void sensor_ssenif_timeout_manual(SENSOR_ID id, BOOL en);
