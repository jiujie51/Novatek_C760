...
typedef enum {
	...
	*/
	...
	CSI_CONFIG_ID_CB,               ///< Set callback function.
...
/**
	CSI Call back function.

	This is used by the CSI ISR callback.
*/
typedef void (*CSI_CB)(UINT32 uiEvent);

/*
	CSI Call back id.

	This is used by the CSI ISR backback.

	It can be bit-or of CSI_CB_ID_XXX.
*/
typedef enum {
	CSI_CB_ID_ERR_ECC         = 0x0001,      ///< CSI ECC error
	CSI_CB_ID_ERR_CRC         = 0x0002,      ///< CSI CRC error
	CSI_CB_ID_ERR_INVALID_DI  = 0x0004,      ///< CSI invalid DI
	CSI_CB_ID_ERR_LWC         = 0x0008,      ///< CSI LWC error
	CSI_CB_ID_ERR_SOT         = 0x0010,      ///< CSI SOT sync error
	CSI_CB_ID_ERR_STA         = 0x0020,      ///< CSI state error
	CSI_CB_ID_ERR_ESC         = 0x0040,      ///< CSI escape error
	CSI_CB_ID_ERR_HST         = 0x0080,      ///< CSI error HS timeout
	CSI_CB_ID_ERR_OV          = 0x0100,      ///< CSI error FIFO overflow

	ENUM_DUMMY4WORD(CSI_CB_ID)
} CSI_CB_ID;
