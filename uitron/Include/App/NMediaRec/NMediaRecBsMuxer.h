...
/**
    NMEDIA_BSQINCARD.
	DISABLE       emrloop will save bsdata in dram buffer.
    Enable        emrloop will save bsdata in card space.
    @note Set1.   Set EMR_BSQ_INCARD and ROLLBACKSEC before starting record
                  MovieExe_OnRecStart()
                  {
                      ImageApp_MovieMulti_SetParam(_CFG_REC_ID_1, MOVIEMULTI_PARAM_EMR_BSQ_INCARD, TRUE);
                      ImageApp_MovieMulti_SetParam(_CFG_REC_ID_1, MOVIEMULTI_PARAM_FILE_ROLLBACKSEC, 30);
                      // trigger record
                  }
    @note Ver.    (191212, 1, 06)

	EMRLOOP.
    @note ON.     Change _CFG_MAIN_EMR_BOTH to _CFG_MAIN_EMR_LOOP (emrloop)
                  MovieExe_OnProtectAuto() & MovieExe_OnProtectManual()
                  {
                      gMovie_Rec_Option.emr_on = _CFG_MAIN_EMR_LOOP;
                  }
    @note Trig.   Use ImageApp_MovieMulti_TrigEMR() to rollback bs.
*/
...
#define BSMUXER_FEVENT_BSINCARD     2
...
#define BSMUXER_FOP_READ       0x00001000
...
/**
    ...
    Set customized data tag, address and size to save in the end of file.

	@param[in] tag   Customized data tag
    @param[in] addr  Customized data address
    @param[in] size  Customized data size

    @return void
*/
extern void     NMR_BsMuxer_SetCustomData(UINT32 tag, UINT32 addr, UINT32 size);
...
/**
	...
	@param[in] pos  Encrypto Pos  (0: iframe 1: container 2: both)
	@param[in] mode Encrypto Mode (1: AES128 2: AES256)

	@return void
*/
extern void     NMR_BsMuxer_SetEnCryptoKey(UINT8 *key, UINT32 pos, UINT32 mode);
