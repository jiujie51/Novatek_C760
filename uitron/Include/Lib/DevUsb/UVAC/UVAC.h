...
#define UVAC_VID_FRM_INTERVAL_MAX_CNT     4

typedef struct _UVAC_VID_FRM_TYPE {
	UINT32 width;                                    ///< Width.
	UINT32 height;                                   ///< Height.
	UINT32 intervals[UVAC_VID_FRM_INTERVAL_MAX_CNT]; ///< interval array, in 100 ns units
	UINT32 min_bit_rate;
	UINT32 max_bit_rate;
	UINT32 max_frm_buf_size;
} UVAC_VID_FRM_TYPE;


typedef struct _UVAC_VID_FRM_INFO {
	UINT32 num;                              ///< The number of video frame type array.
	UVAC_VID_FRM_TYPE *p_frm_type_array;     ///< Video resolution.
} UVAC_VID_FRM_INFO;
...
typedef BOOL (*UVAC_RELEASE_CB)(UINT32 id, UINT32 addr);
...
typedef enum _UVAC_CONFIG_ID {
    ...
    UVAC_CONFIG_YUV_FRM_INFO,               ///< Set YUV frame type info, referring to UVAC_VID_FRM_INFO.
    UVAC_CONFIG_DFU_INFO,                   ///< Set DFU function, referring to UVAC_DFU_INFO.
    UVAC_CONFIG_MAX_UVC_DEV_NUM,            ///< Max UVC device num.
    UVAC_CONFIG_RELEASE_CALLBACK,           ///< The callback function for release buffer
    UVAC_CONFIG_UVC_DIRECT_TRIG,            ///< Transfer will be triggerd by UVAC_SetEachStrmInfo directly instead of timer in 1/fps.
    UVAC_CONFIG_MAX_AUDIO_FRAME_SIZE,       ///< Max audio frame size, the default is 4KB.
    UVAC_CONFIG_CLEAR_VID_DEV_INFO,         ///< clear video device video info.
