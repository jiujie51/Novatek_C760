...
CHKPNT;
		...
		#if 1//(defined(_Disp_IFPA_LCD1_AM350Y10_54E_N02_))    
        	 // 9 bits per transmission
         	sif_setConfig((SIF_CH)DevIOCtrl.SEL.GET_REG_IF.uiSifCh, SIF_CONFIG_ID_LENGTH,   9);
		#else
	...
	if (DevIOCtrl.SEL.GET_PANEL_ADJUST.pfpAdjust != NULL) {CHKPNT;
CHKPNT;
		DevIOCtrl.SEL.GET_PANEL_ADJUST.pfpAdjust();
	} else if (pMode->pCmdQueue != NULL) {CHKPNT;
CHKPNT;
