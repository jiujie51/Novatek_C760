...
static UINT32 motorLV8044_GetInitState(void);
...
static void   motorLV8044_FocusZoomMove(BOOL far_dir, INT32 fsteps, INT32 fbacklash, BOOL tele_dir, INT32 zsteps, INT32 zbacklash);
...
PMOTOR_TAB DrvLens_GetMotor(void)
{
    ...
    g_LensMotorInfo.LensMotor.getInitState       = motorLV8044_GetInitState;
    ...
    g_LensMotorInfo.LensMotor.focus_zoom_setMove = motorLV8044_FocusZoomMove;
...
static void LV8044_SetMotorMove(INT32 focus_steps, INT32 zoom_steps)
{
    UINT32 base_tmp;
    PWM_CFG fpwm_cfg = { 60, 0, 30, 1, 0 }, zpwm_cfg = { 60, 0, 30, 1, 0 };    // uiPrd, uiRise, uiFall, uiOnCycle (0 ~ 65535), uiInv
    ...
    if (focus_steps != 0) {
        ...
        LV8044_TransmitCmd(pfocus_motor_cfg->eChnelSel, LV8044_CMD_DIRECTION, pfocus_motor_cfg->iMoveDir);
    }
    if (zoom_steps != 0) {
        pzoom_motor_cfg->iWorkStatus = pzoom_motor_cfg->iMotorStatus = MOTOR_STATUS_BUSY;
        LV8044_TransmitCmd(pzoom_motor_cfg->eChnelSel, LV8044_CMD_DIRECTION, pzoom_motor_cfg->iMoveDir);
    }

    LENS_DELAY_MS(2);    // 2 msec delay for direction cmd

    focus_steps = LENS_ABS(focus_steps);
    zoom_steps = LENS_ABS(zoom_steps);

    switch (pfocus_motor_cfg->eMicroStepMode) {
    case LV8044_MODE_2PHASE:
        focus_steps *= 2;
        break;
    ...
    switch (pzoom_motor_cfg->eMicroStepMode) {
    case LV8044_MODE_2PHASE:
        zoom_steps *= 2;
        break;
    ...
    if (focus_steps != 0) {
        base_tmp = (focus_steps > zoom_steps) ?  pfocus_motor_cfg->uiPwmBaseNum : (zoom_steps * pfocus_motor_cfg->uiPwmBaseNum / focus_steps);
        fpwm_cfg.uiPrd = LENS_CLAMP(base_tmp, 15, 240);
        fpwm_cfg.uiFall = fpwm_cfg.uiPrd / 2;
        fpwm_cfg.uiOnCycle = focus_steps;
        pwm_pwmConfig(pfocus_motor_cfg->uiPwmOutputPin, &fpwm_cfg);
    }
    if (zoom_steps != 0) {
        base_tmp = (zoom_steps > focus_steps) ? pzoom_motor_cfg->uiPwmBaseNum : (focus_steps * pzoom_motor_cfg->uiPwmBaseNum / zoom_steps);
        zpwm_cfg.uiPrd = LENS_CLAMP(base_tmp, 15, 240);
        zpwm_cfg.uiFall = zpwm_cfg.uiPrd / 2;
        zpwm_cfg.uiOnCycle = zoom_steps;
        pwm_pwmConfig(pzoom_motor_cfg->uiPwmOutputPin, &zpwm_cfg);
    }

    if (focus_steps != 0) {
        pwm_pwmEnable(pfocus_motor_cfg->uiPwmOutputPin);
    }
    if (zoom_steps != 0) {
        pwm_pwmEnable(pzoom_motor_cfg->uiPwmOutputPin);
    }

    if (focus_steps != 0) {
        pwm_wait(pfocus_motor_cfg->uiPwmOutputPin, PWM_TYPE_PWM);
    }
    if (zoom_steps != 0) {
        pwm_wait(pzoom_motor_cfg->uiPwmOutputPin, PWM_TYPE_PWM);
    }

    pfocus_motor_cfg->iMotorStatus = pzoom_motor_cfg->iMotorStatus = MOTOR_STATUS_FREE;

    if ((pfocus_motor_cfg->bSetFinalStep == TRUE) || (pzoom_motor_cfg->bSetFinalStep == TRUE)) {
        LENS_DELAY_MS(3);    // 3 msec delay for power down

        if (pfocus_motor_cfg->bSetFinalStep == TRUE) {
            LV8044_TransmitCmd(pfocus_motor_cfg->eChnelSel, LV8044_CMD_SET_CURR, 3);
            pfocus_motor_cfg->iWorkStatus = MOTOR_STATUS_FREE;
            pfocus_motor_cfg->bSetFinalStep = FALSE;
        }
        if (pzoom_motor_cfg->bSetFinalStep == TRUE) {
            LV8044_TransmitCmd(pzoom_motor_cfg->eChnelSel, LV8044_CMD_SET_CURR, 3);
            pzoom_motor_cfg->iWorkStatus = MOTOR_STATUS_FREE;
            pzoom_motor_cfg->bSetFinalStep = FALSE;
        }
    }
}
...
// zoom
//----------------------------------------------------------------------------------------------------------------------
static INT32 LV8044_ZoomMove(BOOL tele_dir, INT32 steps, INT32 gear_backlash)
{
    INT32 set_step = 0;
...
static void motorLV8044_ZoomSetState(MOTOR_ZOOM_ACT state, UINT32 param)
{
    ...
    switch (state) {
    case MOTOR_ZOOM_FWD:
        LV8044_ZoomMove(TRUE, (INT32)(param & 0xFFFF), (INT32)((param >> 16) & 0xFFFF));
        break;

    case MOTOR_ZOOM_BWD:
        LV8044_ZoomMove(FALSE, (INT32)(param & 0xFFFF), (INT32)((param >> 16) & 0xFFFF));
        break;
...
// focus
//----------------------------------------------------------------------------------------------------------------------
static INT32 LV8044_FocusMove(BOOL far_dir, INT32 steps, INT32 gear_backlash)
{
    INT32 set_step = 0;
...
static void motorLV8044_FocusZoomMove(BOOL far_dir, INT32 fsteps, INT32 fbacklash, BOOL tele_dir, INT32 zsteps, INT32 zbacklash)
{
    ...
    if ((far_dir == TRUE) && (tele_dir == TRUE)) {
        LV8044_SetMotorMove(fset_step, zset_step);
...
static BOOL motorLV8044_FocusSetState(MOTOR_FOCUS_ACT state, UINT32 param)
{
    ...
    case MOTOR_FOCUS_FWD:
        LV8044_FocusMove(TRUE, (INT32)(param & 0xFFFF), (INT32)((param >> 16) & 0xFFFF));
        break;

    case MOTOR_FOCUS_BWD:
        LV8044_FocusMove(FALSE, (INT32)(param & 0xFFFF), (INT32)((param >> 16) & 0xFFFF));
        break;
