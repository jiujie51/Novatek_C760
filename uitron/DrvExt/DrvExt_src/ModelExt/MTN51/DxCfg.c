...
extern DX_OBJECT gDevEmbMem9;
extern DX_OBJECT gDevEmbMem10;
...
DX_HANDLE Dx_GetObject(UINT32 DxClassType)  // Query device object
{
	...
	case DX_CLASS_STORAGE_EXT:
		...
		} else if ((DxClassType & DX_TYPE_MASK) == DX_TYPE_EMBMEM9) {
			hDevice = (UINT32)(&gDevEmbMem9);
		} else if ((DxClassType & DX_TYPE_MASK) == DX_TYPE_EMBMEM10) {
			hDevice = (UINT32)(&gDevEmbMem10);
		}
		#endif
