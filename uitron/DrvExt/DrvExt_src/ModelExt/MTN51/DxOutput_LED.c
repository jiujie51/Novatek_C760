...
void LED_TurnOnLED(UINT32 uiLED)
{
	...
	case GPIOMAP_LED_MOVIE:
		gpio_clearPin(GPIO_MOVIE_LED);
		break;

	case GPIOMAP_LED_STATUS:
		gpio_clearPin(GPIO_STATUS_LED);
		break;
	#endif
...
BOOL LED_IsLEDOn(UINT32 uiLED)
{
	...
	#if 0
	case GPIOMAP_LED_MOVIE:
		bvalue = !gpio_getPin(GPIO_MOVIE_LED);
		break;

	case GPIOMAP_LED_STATUS:
		bvalue = !gpio_getPin(GPIO_STATUS_LED);
		break;
	#endif
