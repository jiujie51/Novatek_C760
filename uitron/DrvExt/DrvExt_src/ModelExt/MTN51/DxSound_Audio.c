...
DRVSOUND_OUTDEV_CONFIG gDxSndAudOutDevConfigTbl[DRVSOUND_OUTDEV_CONFIG_CNT] = {
	...
	{DRVSOUND_VOUT_TV, AUDIO_CH_LEFT, FALSE},
	{DRVSOUND_VOUT_HDMI, AUDIO_CH_STEREO, FALSE},
	{DRVSOUND_VOUT_LCDTV, AUDIO_CH_LEFT, FALSE},
...
ER DrvSound_SetOutDevConfig(UINT32 outDevIdx, PAUDTS_OBJ pAudPlayObj)
{
	...
	if (gpCurDrvAudOutDev->vidOutType == DRVSOUND_VOUT_LCD) {
		...
		aud_setFeature(AUDIO_FEATURE_LINE_DEPOP_EN, TRUE);
		aud_setFeature(AUDIO_FEATURE_HP_DEPOP_EN, TRUE);
...
void DrvSound_SetAudioDevInitData(AUDIO_DEVICE_OBJ  *pAudioDevice, AUDIO_SETTING *pAudioSetting)
{
	...
	if (pAudioDevice) {
		...
		pAudioDevice->uiGPIOData          = 32;//P_GPIO_3;

		//pAudioDevice->uiGPIOClk           = 0;
		pAudioDevice->uiGPIOClk           = 32;//P_GPIO_2;
...
void DrvSound_SetAudioFeature(void)
{
	...
	aud_setParameter(AUDIO_PARAMETER_BOOST_GAIN, EAC_PGABOOST_SEL_30DB);
...
void DrvSound_CtrlPwr(BOOL bOutput);
void DrvSound_CtrlPwr(BOOL bOutput)
{
    if(bOutput)
		gpio_setPin(GPIO_SPK_PWR_CTRL);
	else
		gpio_clearPin(GPIO_SPK_PWR_CTRL);
}

BOOL DrvSound_IsPwrOn(void);
BOOL DrvSound_IsPwrOn(void)
{
    return gpio_getPin(GPIO_SPK_PWR_CTRL);
}
//=============
