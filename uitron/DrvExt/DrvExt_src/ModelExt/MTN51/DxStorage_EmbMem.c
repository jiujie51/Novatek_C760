...
UINT32 DrvEmbMemGetcaps9(UINT32 CapID, UINT32 Param1); // Get Capability Flag (Base on interface version)
UINT32 DrvEmbMemGetcaps10(UINT32 CapID, UINT32 Param1); // Get Capability Flag (Base on interface version)
...
DX_OBJECT gDevEmbMem9 = {
	DXFLAG_SIGN,
	DX_CLASS_STORAGE_EXT | DX_TYPE_EMBMEM9,
	STORAGE_VER,
	"USER0",
	0, 0, 0, 0,
	DrvEmbMemGetcaps9,
...
DX_OBJECT gDevEmbMem10 = {
	DXFLAG_SIGN,
	DX_CLASS_STORAGE_EXT | DX_TYPE_EMBMEM10,
	STORAGE_VER,
	"USER1",
	0, 0, 0, 0,
	DrvEmbMemGetcaps10,
...
UINT32 DrvEmbMemGetcaps9(UINT32 CapID, UINT32 Param1)
{
	return xDrvEmbMemGetcaps(DX_TYPE_EMBMEM9, CapID, Param1);
}
UINT32 DrvEmbMemGetcaps10(UINT32 CapID, UINT32 Param1)
{
	return xDrvEmbMemGetcaps(DX_TYPE_EMBMEM10, CapID, Param1);
}
...
UINT32 DrvEmbMemInit(void *pInitParam) // Set Init Parameters
{
...

	{
		...
		if (p->uiDxClassType == (DX_CLASS_STORAGE_EXT | DX_TYPE_EMBMEM0)) {
			...
			debug_msg("Liwk -------- SPIFLASH BLOCK SIZE:%d\r\n",blksize );
