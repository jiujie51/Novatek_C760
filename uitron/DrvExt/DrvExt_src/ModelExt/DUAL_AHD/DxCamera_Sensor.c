...
#if (defined(_CMOS_AD_n4_))
static SENSOR_INIT_OBJ sensor_get_obj_ad_n4(SENSOR_ID id)
{
	// set sensor command interface info (Vx1/SIF/I2C/IO/...)
	SENSOR_INIT_OBJ init_obj = {0};
	init_obj.cmd_info.cmd_type = SENSOR_CMD_I2C;
...
static void sen_power_ctrl_ad_n4(SENSOR_ID id, POW_CTRL_FLAG flag)
{
	switch (flag) {
	case POW_CTRL_TURN_ON:
		CHKPNT;
		...
		pll_selectClkSrc(PLL_CLK_SPCLK, PLL_CLKSRC_PLL5);
		pll_setClkFreq(PLL_CLK_SPCLK, 27000000);
		pll_setClkEn(PLL_CLK_SPCLK, ENABLE);
...
static BOOL sen_det_plug_in_ad_n4(SENSOR_ID id)
{
	BOOL rt = E_OK;
	SENSOR_HANDLE sen_handle = {0};
...
#endif
#if (defined(_CMOS_AD_n4_))
	{"CMOS_AD_n4", sensor_get_drv_tab_ad_std_drv_n4, sensor_get_obj_ad_n4, sen_power_ctrl_ad_n4, sen_det_plug_in_ad_n4, NULL, NULL},
#endif
...
BOOL drv_sensor_first_cfg(void)
{
	...
	sensor_set_group_rec(sensor_gp_rec, sensor_subgp_rec, senname_idx);
	sensor_set_ad_ext_info(sensor_ad_ext_info);
...
void drv_sensor_power_turn_on(SENSOR_ID id)
{
	...
	if (sensor_chk_power_on(id) && (b_powere_on_ready==FALSE)) {
		CHKPNT;
		//b_powere_on_ready = FALSE;
		sensor_info->pow_ctrl(id, POW_CTRL_TURN_ON);
		b_powere_on_ready = TRUE;
	}
}
