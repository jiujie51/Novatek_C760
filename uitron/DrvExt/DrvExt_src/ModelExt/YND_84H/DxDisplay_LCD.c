...
#define LCD_ASPECT_W                    1 //9
#define LCD_ASPECT_H                    4 //32
...
static UINT32 DrvLCDState(UINT32 StateID, UINT32 Value) // General Properties
{
	...
	if (StateID & DXGET) {
		...
		case DRVDISP_STATE_DIRECT:
			...
			//NOTE: Refer to MIRROR_DISCARD, MIRROR_X, MIRROR_Y and MIRROR_KEEP in GxDisplay.h
			if(pObj) {
				DISPLAYER_PARAM LyrParam;
				memset(&LyrParam, 0, sizeof(DISPLAYER_PARAM));
				if (((Value & 0x01) == 0x01) && ((Value & 0x02) == 0x02)) {  // MIRROR_X | MIRROR_Y
					LyrParam.SEL.SET_OUTDIR.BufOutDir = DISPOUTDIR_ROT_180;
				}else if ((Value & 0x01) == 0x01) {  // MIRROR_X
					LyrParam.SEL.SET_OUTDIR.BufOutDir = DISPOUTDIR_HRZ_FLIP;
				}else if ((Value & 0x02) == 0x02) {  // MIRROR_Y
					LyrParam.SEL.SET_OUTDIR.BufOutDir = DISPOUTDIR_VTC_FLIP; // If the LCD is vertical display, please set this.	
				} else {
				...
				if ((Value & 0xff) == 0xff){ //MIRROR_KEEP
				pObj->load(TRUE);
			}
			}
			#endif
