...
void IO_InitADC(void)
{
...
#if 1
	if (adc_open(ADC_CH_VOLDET_BATTERY_MT) != E_OK) {
		DBG_ERR("Can't open ADC channel for battery voltage detection\r\n");
		return;
	}
	...
	//battery voltage detection
	adc_setChConfig(ADC_CH_VOLDET_BATTERY_MT, ADC_CH_CONFIG_ID_SAMPLE_FREQ, 10000); //10K Hz, sample once about 100 us for CONTINUOUS mode
	adc_setChConfig(ADC_CH_VOLDET_BATTERY_MT, ADC_CH_CONFIG_ID_SAMPLE_MODE, (VOLDET_ADC_MODE) ? ADC_CH_SAMPLEMODE_CONTINUOUS : ADC_CH_SAMPLEMODE_ONESHOT);
	adc_setChConfig(ADC_CH_VOLDET_BATTERY_MT, ADC_CH_CONFIG_ID_INTEN, FALSE);
#endif
