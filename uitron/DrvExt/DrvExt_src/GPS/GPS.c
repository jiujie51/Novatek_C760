...
extern void Gps_ShowOSDString(CHAR *pString, UINT16 uiX, UINT16 uiY, UINT16 uiColor);
...
void GPSRec_NMEAParser(CHAR *NMEA_str, UINT32 Len, NMEATYPE NMEAtype)
{
    ...
    case RMC:
		...
		if(Status == 'A')   //Data valid
        {
			...
			/*DBG_DUMP("\x1b[34m");   //Blue
            DBG_DUMP("#RMC: Status = %c, Speed = %.1f, Angle = %.1f, Latitude = %.1f, Longitude = %.1f \r\n",RMCInfo.Status,RMCInfo.Speed,RMCInfo.Angle,RMCInfo.Latitude,RMCInfo.Longitude);
            DBG_DUMP("#RMC: Date = %d\\%d\\%d CheckSum = %X\r\n",RMCInfo.Year,RMCInfo.Month,RMCInfo.Day,CheckSum);
...
//pqw add:Get sat nums
UINT32 GPSRec_GetGSVSatInView(void)
{
	return GSVInfo.SatInView;
}
